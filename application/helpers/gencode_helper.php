...
function FCNaHGenCodeV5($ptTable = '',$ptStaDocType = '0',$ptBchCode = ''){
	...
	if(isset($ptBchCode) && !empty($ptBchCode)){
		...
		if(!empty($_SESSION["tSesUsrBchCode"])){
			$tSesUsrBchCode	= $_SESSION["tSesUsrBchCode"];
		}else{
			$tSesUsrBchCode	= $ci->input->post("ohdSOSesUsrBchCode");
		}
	...
	$tSesUsrShpCode	= $ci->input->post("tSesUsrShpCode");
	...
	if($nRowHisMas > 0){
		...
		if(strlen($tHisMasAll) == $nAutoAll AND strlen($tHisMasNum) == $nAutoNum) {
			...
			if(!empty($_SESSION['tSesUsername'])){
					$tSesUsername = $_SESSION['tSesUsername'];
			}else{
	...
	if($tCheckDup == 0){
		...
		return FCNaHGenCodeV5($ptTable,$ptStaDocType,$ptBchCode);
		// return array(
		// 	$rtGenCode => null,
		// 	'rtCode' => '905',
