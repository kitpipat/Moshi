    ...
    if($pnRowPartID == 1){
        ...
        for($i = 0;$i<count($paGrouppingData);$i++){
            if($paGrouppingData[$i] == $paGrouppingData[0] ){
                echo "<td class='xCNRptGrouPing  text-left' colspan='6' style='padding: 5px;'>".$paGrouppingData[$i]."</td>";
            }else{
...
//มอส
// สั่ง Grouping รายงาน Level3
function FCNtHRPTHeadGrouppingRptLevel3($pnRowPartID,$paGrouppingData){
    if($pnRowPartID == 1){
        echo '<tr style="border-top: solid 1px #333 !important;">';
        for($i = 0;$i<count($paGrouppingData);$i++){
            if($paGrouppingData[$i] == $paGrouppingData[0] ){
                echo "<td class='xCNRptGrouPing  text-left' colspan='3' style='padding: 5px;'>".$paGrouppingData[$i]."</td>";
            }else{
...
//มอส
// สั่ง Grouping รายงาน Level4
function FCNtHRPTHeadGrouppingRptLevel4($pnRowPartID,$paGrouppingData){
    if($pnRowPartID == 1){
        echo '<tr style="border-top: solid 0px #333 !important;">';
        for($i = 0;$i<count($paGrouppingData);$i++){
            if($paGrouppingData[$i] == $paGrouppingData[0] ){
                echo "<td class='xCNRptGrouPing  text-left' colspan='3' style='padding: 5px;'>".$paGrouppingData[$i]."</td>";
            }else{
...
//มอส
// สั่ง Grouping รายงาน Level5
function FCNtHRPTHeadGrouppingRptLevel5($pnRowPartID,$paGrouppingData){
    if($pnRowPartID == 1){
        echo '<tr style="border-top: solid 1px #333 !important;">';
        for($i = 0;$i<count($paGrouppingData);$i++){
            if($paGrouppingData[$i] == $paGrouppingData[0] ){
...
function FCNoHDOCClearRptTmp()
{
    ...
    foreach ($aTableReportTmp as $tItem) {
        $tSQL .= "
            DELETE FROM $tItem WHERE FDTmpTxnDate < ( SELECT CURRENT_TIMESTAMP - 1 DAYS )
        ";
    }
...
function FCNnGetNumeric($pnVal) {
    if (is_numeric($pnVal)) {
      ...
      $nValue = str_replace(",","",number_format($pnVal,$nOptDecimalShow))+0;
