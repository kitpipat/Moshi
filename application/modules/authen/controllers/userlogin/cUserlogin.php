...
class cUserlogin extends MX_Controller {
    ...
    public function  FSaCUserlogAddEvent(){
        try{    
            ...
            $aDataMaster    = array(
                ...
                'FTUsrStaActive'    => (!empty($this->input->post('ocbUsrlogStaUse')))? '1':'2',
            );
    ...
    //Return : array
    //Return Type : array
    public function FSoCUserlogChangePasswordEvent(){

        $tSesUsrLogin  =  $this->input->post('tSesUsrLogin');
        $tPasswordOld  =  $this->input->post('oetPasswordOld');
        $tPasswordNew  =  $this->input->post('oetPasswordNew');

        $aResult = $this->mUserlogin->FSoMUSRLCheckPassOld($tSesUsrLogin,$tPasswordOld);
        if($aResult==true){
            $aDataParam = array(
                'FTUsrCode' => $this->session->userdata('tSesUserCode'),
                'FTUsrLogin' => $tSesUsrLogin,
                'FTUsrLoginPwd' => $tPasswordNew,
                'FDLastUpdOn' => date('Y-m-d H:i:s'),
                'FTLastUpdBy' => $this->session->userdata('tSesUserCode'),
            );
            $this->mUserlogin->FSoMUSRLChangePasswordNew($aDataParam);
            $aDataReturn     = array(
                'nStaEvent' => '01',
                'tStaMessg' => 'Success'
            );
