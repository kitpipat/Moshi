...
class mUserlogin extends CI_Model {
    ...
    public function  FSaMUSRLAddUpdateMaster($paData){
        ...
        try{
            ...
            $this->db->where('FDUsrPwdStart'  , $paData['FDUsrPwdStart']);
    ...
    //Functionality : Checkduplicate Data 
    //Parameters : function parameters
    //Creator :  27/02/2021 Nattakit
    //Last Modified : -
    //Return : data Count Duplicate
    //Return Type : object
    public function FSoMUSRLCheckPassOld($ptUsrLogin,$tPassOld){
        $tSQL   = "SELECT COUNT(FTUsrLogin)AS counts
                   FROM TCNMUsrLogin
                   WHERE FTUsrLogin = '$ptUsrLogin' 
                     AND FTUsrLoginPwd = '$tPassOld' ";
        $oQuery = $this->db->query($tSQL);
        $nResult = $oQuery->row_array()["counts"];
        if($nResult>0){
            return true;
    ...
    //Functionality : ChangePasswordNew
    //Parameters : function parameters
    //Creator :  27/02/2021 Nattakit
    //Last Modified : -
    //Return : data success
    //Return Type : object
    public function FSoMUSRLChangePasswordNew($paData){

            $tUsrCode = $paData['FTUsrCode'];
            $tUsrLogin = $paData['FTUsrLogin'];
            $tUsrLoginPwd = $paData['FTUsrLoginPwd'];
            $dLastUpdOn = $paData['FDLastUpdOn'];
            $tLastUpdBy = $paData['FTLastUpdBy'];

        //Update UsrLogin
        $this->db->set('FTUsrLoginPwd' , $tUsrLoginPwd);
        $this->db->where('FTUsrCode'  , $tUsrCode);
        $this->db->where('FTUsrLogin'  , $tUsrLogin);
        $this->db->update('TCNMUsrLogin');
