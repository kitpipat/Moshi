<?php
    ...
    if(isset($nStaAddOrEdit) && $nStaAddOrEdit == 1){
        ...
        $nBNTStaShw     = $aBntData['raItems']['rtBntStaShw'];
    }else{
...
<form class="validate-form" action="javascript:void(0)" method="post" enctype="multipart/form-data" id="ofmAddBnt">
    <button style="display:none" type="submit" id="obtSubmitBnt" onclick="JSoAddEditBnt('<?= $tRoute?>')"></button>
    <div class="panel-body" style="padding-top:20px !important;"> <!-- เพิ่มมาใหม่ -->
        <div class="row">   
            ...
            <div class="col-xs-12 col-md-5 col-lg-5">
                ...
                <div class="form-group">
                    <label class="xCNLabelFrm"><?= language('payment/banknote/banknote','tAmount')?></label> <!-- เปลี่ยนชื่อ Class -->
                    <input type="text" class="form-control xCNInputNumericWithDecimal text-right" maxlength="50" id="oetBntAmt" name="oetBntAmt" onclick="JCNdValidateComma('oetBntAmt',4, 'FC');" onfocusout="JCNdValidatelength8Decimal('oetBntAmt','FC',4,'<?php echo $tDecSave?>')"  value="<?=$tBnnAmt?>" 
                ...
                <div class="form-group">
                    <label class="fancy-checkbox">
                        <?php
                            if(isset($nBNTStaShw) && $nBNTStaShw == 1){
                                $tCheckednBNTStaShw  = 'checked';
                            }else{
                                $tCheckednBNTStaShw  = '';
                            }
                        ?>
                        <input type="checkbox" id="ocbBntStaShw" name="ocbBntStaShw" value="1" <?php echo $tCheckednBNTStaShw;?>>
                        <span> <?php echo language('payment/recivespc/recivespc','tBntStaShw');?></span>
                    </label>
                </div>
