...
class mFuncSetting extends CI_Model {
    ...
    public function FSaMFuncSetingGetDataHD($paParams = []){
        ...
        $tSQL = "
            SELECT C.* FROM(
                SELECT  ROW_NUMBER() OVER(ORDER BY FTAppName ASC, FTGdtName ASC) AS FNRowID,* FROM
                    (SELECT DISTINCT
                        HD.FTGhdApp,
                        APL.FTAppName,
                        DTL.FTGdtName,
    ...
    public function FSaMFuncSettingInsertDTToTemp($paParams = []){
        ...
        $tSQL = "   
            INSERT TsysMasTmp WITH (ROWLOCK)
                (FTGhdApp, FTAppName, FTGdtName, /* FTGhdCode, FTSysCode, */ FTGdtCallByName, FNGdtFuncLevel, FTGdtStaUse, FTMttTableKey, FTMttRefKey, FTMttSessionID)
    ...
    public function FSaMFuncSettingUpdateTmpToDT($paParams = []){
        ...
        $tSQL = "   
            ...
            WHERE TPSMFuncDT.FTGdtCallByName = TMP.FTGdtCallByName
            AND TPSMFuncDT.FTGhdCode = (
                SELECT 
                    FTGhdCode	
    ...
    public function FSaMFuncSettingGetSystemAppAll($paParams = []){
        ...
        $tSQL = "   
            SELECT DISTINCT 
                ...
                APL.FTAppName
            FROM TPSMFuncHD HD 
            INNER JOIN TPSMFuncDT DT ON HD.FTGhdCode = DT.FTGhdCode
            /* ส่งพารามิตเตอร์ ภาษา */
            INNER JOIN TPSMFuncDT_L DTL ON DT.FTGhdCode = DTL.FTGhdCode AND DT.FTSysCode = DTL.FTSysCode AND DTL.FNLngID = $nLngID
            INNER JOIN TSysApp_L APL ON HD.FTGhdApp = APL.FTAppCode AND APL.FNLngID = $nLngID
