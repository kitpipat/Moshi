...
class mDashBoardSale extends CI_Model
{
    ...
    public function FSaMDSHSALTopTenBestSeller($paDataWhere)
    {
        ...
        if (isset($paDataWhere['tFilterStaCst']) && !empty($paDataWhere['tFilterStaCst'])) {
            if ($paDataWhere['tFilterStaCst'] == '1') {
                $tTextWhereCstHave   = ' AND FNStaCstHave > 0';
            } else {
                $tTextWhereCstHave   = ' AND FNStaCstHave = 0';
            }
        }
        ...
        $tSQL   = " SELECT 
                    ...
                    ) AS TOPBESTSELL ORDER BY TOPBESTSELL.FCXsdNet DESC";
        ...
        (
            ...
            WHERE HD.FTXshStaDoc = 1
                  AND DT.FTXsdStaPdt <> 4
                       ...
                       PDTL.FTPdtName ) S ORDER BY FCXsdNet DESC";
    ...
    (
        ...
        (
            ...
            (
                ...
                FROM TPSTSalHD HD
                     LEFT JOIN TPSTShiftSLastDoc SHD ON HD.FTBchCode = SHD.FTBchCode
                                                         AND HD.FTShfCode = SHD.FTShfCode
                                                         AND HD.FNXshDocType = SHD.FNLstDocType
                WHERE CONVERT(DATE, HD.FDXshDocDate, 103) BETWEEN '" . $dDateFrom . "' AND  '" . $dDateTo . "' 
                 $tTextWhereBranch 
                 $tTextWherePos
...
// print_r($tSQL);
        ...
        if ($oQuery->num_rows() > 0) {
            ...
            $nFoundRow = $this->FSoMSPLGetPageAll($dDateFrom, $dDateTo, $tTextWhereBranch, $tTextWherePos, $tTextWhereDiff, $oetDSHSALSort, $oetDSHSALFild);
    ...
    // Return: Data Int
    // Return Type: Int
    public function FSoMSPLGetPageAll($pdDateFrom, $pdDateTo, $ptTextWhereBranch, $ptTextWherePos, $tTextWhereDiff, $oetDSHSALSort, $oetDSHSALFild)
    {
            ...
            (
                ...
                FROM TPSTSalHD HD
                     LEFT JOIN TPSTShiftSLastDoc SHD ON HD.FTBchCode = SHD.FTBchCode
                                                         AND HD.FTShfCode = SHD.FTShfCode
                                                         AND HD.FNXshDocType = SHD.FNLstDocType
                WHERE CONVERT(DATE, HD.FDXshDocDate, 103) BETWEEN '" . $pdDateFrom . "' AND  '" . $pdDateTo . "'  
                GROUP BY HD.FTBchCode, 
                         HD.FTPosCode, 
                         CONVERT(DATE, HD.FDXshDocDate, 103), 
                         HD.FTShfCode, 
