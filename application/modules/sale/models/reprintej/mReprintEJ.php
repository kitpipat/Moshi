...
class mReprintEJ extends CI_Model {
    ...
    public function FSaMGetListDataEJ($paDataWhere){
        ...
        if(isset($tShopCode) && !empty($tShopCode)){
            $tWhereEJShop = " 
                AND (EJ.FTShpCode = '$tShopCode')
            ";
        }

        $tWhereEJDocDate = "";
        $tSearchDocDateFrom = $aDataFilterEJ['oetEJDocDateFrom'];
        $tSearchDocDateTo = $aDataFilterEJ['oetEJDocDateTo'];
        
        if (!empty($tSearchDocDateFrom) && !empty($tSearchDocDateTo)) {
            $tWhereEJDocDate = " AND ((EJ.FDXshDocDate BETWEEN CONVERT(datetime,'$tSearchDocDateFrom 00:00:00') AND CONVERT(datetime,'$tSearchDocDateTo 23:59:59')) OR (EJ.FDXshDocDate BETWEEN CONVERT(datetime,'$tSearchDocDateTo 00:00:00') AND CONVERT(datetime,'$tSearchDocDateFrom 23:59:59')))";
        }

        // Filter Document Single
        $tDocNoSingle = $aDataFilterEJ['oetEJSlipCode'];
        $tWhereEJDocNoSingle = "";
        if(isset($tDocNoSingle) && !empty($tDocNoSingle)){
            $tWhereEJDocNoSingle = " 
                AND (EJ.FTXshDocNo = '$tDocNoSingle')
            ";
        }

        // Filter Document Range
        ...
        if((isset($tDocNoFrom) && !empty($tDocNoFrom)) && (isset($tDocNoTo) && !empty($tDocNoTo))){
            $tWhereEJDocNoBetween = " 
                AND ((EJ.FTXshDocNo BETWEEN '$tDocNoFrom' AND '$tDocNoTo') OR (EJ.FTXshDocNo BETWEEN '$tDocNoTo' AND '$tDocNoFrom'))
            ";
        }

        // Filter Doc Type
        $tDocType = $aDataFilterEJ['ocmEjDocumentType'];
        $tWhereEJDocType = "";
        if(!empty($tDocType)){
            switch($tDocType){
                case 'S': {
                    $tWhereEJDocType = " AND LEFT(EJ.FTXshDocNo, 1) = 'S' AND LEFT(EJ.FTXshDocNo, 3) <> 'SUM'";
                    break;
                }
                case 'RCV': {
                ...
                case 'R': {
                    $tWhereEJDocType = " AND LEFT(EJ.FTXshDocNo, 1) = 'R' AND LEFT(EJ.FTXshDocNo, 3) <> 'RCV'";
                    break;
                }
                case 'BNK': {
                    $tWhereEJDocType = " AND LEFT(EJ.FTXshDocNo, 3) = 'BNK'";
                    break;
                }
            }
        }
