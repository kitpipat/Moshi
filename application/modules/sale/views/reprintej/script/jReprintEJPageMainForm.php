<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#ocmEjDocumentType").on("change", function(){
            var tDocType = $(this).val();
            var tEJLabel = '<?php echo language('sale/reprintej/reprintej','tEJPanelAbbrViewer');?>';

            if(tDocType != ""){
                var tDocTypeLabel = $(this).find('option[value='+tDocType+']').text();
                tEJLabel += " ("+tDocTypeLabel+")";
            }

            switch(tDocType){
                case 'S': {
                    $("#odvEJAbbrViewer label.xCNTextDetail1").text(tEJLabel);
                    break;
                }
                case 'RCV': {
                    $("#odvEJAbbrViewer label.xCNTextDetail1").text(tEJLabel);
    ...
    // ======================================== Browse Option ========================================
        ...
        var oEJSlipOption = function(poReturnInput){
            ...
            var tDateFrom = $("#oetEJDocDateFrom").val();
            var tDateTo = $("#oetEJDocDateTo").val();
            var tDocType = $("#ocmEjDocumentType").val();
            var tWhereBranch = '';
            var tWhereShop = '';
            var tWhereDate = '';
            var tWhereDocType = '';
            ...
            if((typeof tDateFrom !== 'undefined' && tDateFrom != '') && (typeof tDateTo !== 'undefined' && tDateTo != '')){
                tWhereDate = " AND ((TPSTSlipEJ.FDXshDocDate BETWEEN CONVERT(datetime,'"+tDateFrom+" 00:00:00') AND CONVERT(datetime,'"+tDateTo+" 23:59:59')) OR (TPSTSlipEJ.FDXshDocDate BETWEEN CONVERT(datetime,'"+tDateTo+" 00:00:00') AND CONVERT(datetime,'"+tDateFrom+" 23:59:59')))";
            }

            if(typeof tDocType !== 'undefined' && tDocType != ''){
                switch(tDocType){
                    case 'S': {
                        tWhereDocType = " AND (LEFT(TPSTSlipEJ.FTXshDocNo, 1) = 'S' AND LEFT(TPSTSlipEJ.FTXshDocNo, 3) <> 'SUM')";
                        break;
                    }
                    case 'RCV': {
                    ...
                    case 'R': {
                        tWhereDocType = " AND (LEFT(TPSTSlipEJ.FTXshDocNo, 1) = 'R' AND LEFT(TPSTSlipEJ.FTXshDocNo, 3) <> 'RCV')";
                        break;
                    }
                    case 'BNK': {
                        tWhereDocType = " AND (LEFT(TPSTSlipEJ.FTXshDocNo, 3) = 'BNK')";
                        break;
                    }
                }
            }
            ...
            var oOptionReturn = {
                ...
                Join    : {
                    ...
                    On: [
                        ...
                        "TCNTAuto_L.FTSatStaDocType = TPSTSalHD.FNXshDocType AND TCNTAuto_L.FTSatTblName ='TPSTSalHD' AND TCNMUser_L.FNLngID = "+nEJLangEdits
                    ]
                },
                Where :{
                    Condition : [tWhereBranch+tWhereShop+tWhereDocType+tWhereDate]
                },
                GrideView:{
