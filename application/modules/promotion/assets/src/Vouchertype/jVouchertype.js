...
$('document').ready(function() {
    ...
    if (nStaVocBrowseType != 1) {
        JSvCallPageVoucherTypeList();
    } else {
        JSvCallPageVouchertypeAdd();
    }
...
function JSnAddEditVoucher(ptRoute) {
    ...
    if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
        $('#ofmAddVouchertype').validate().destroy();
        ...
        submitHandler: function(form) {
            $.ajax({
                type: "POST",
                url: ptRoute,
                data: $('#ofmAddVouchertype').serialize(),
                cache: false,
                timeout: 0,
                    ...
                    if (nStaVocBrowseType != 1) {
                        ...
                        if (aReturn['nStaEvent'] == 1) {
                            if (aReturn['nStaCallBack'] == '1' || aReturn['nStaCallBack'] == null) {
                                JSvCallPageVoucherTypeEdit(aReturn['tCodeReturn'])
...
function JSvCallPageVoucherTypeList() {

    localStorage.tStaPageNow = 'JSvCallPageVoucherTypeList';

    $.ajax({
        type: "GET",
        url: "vouchertypeFormSearchList",
        data: {},
        cache: false,
        timeout: 5000,
        success: function(tResult) {
            ...
            JSvCallPageVoucherTypeDataTable(); //แสดงข้อมูลใน List
        },
        error: function(data) {
            console.log(data);
        }
...
function JSvCallPageVoucherTypeDataTable(pnPage) {
...
//Return : View
//Return Type : View
function JSvCallPageVouchertypeAdd() {
    $.ajax({
        type: "GET",
        url: "VoucherTypePageAdd",
        data: {},
        cache: false,
        timeout: 5000,
        success: function(tResult) {
...
//Return : View
//Return Type : View
function JSvCallPageVoucherTypeEdit(ptVotCode){
    
    JCNxOpenLoading();
    JStCMMGetPanalLangSystemHTML('JSvCallPageVoucherTypeEdit', ptVotCode);

    $.ajax({
        type: "POST",
        url: "vouchertypePageEdit",
        data: { tVotCode : ptVotCode },
        cache: false,
        timeout: 0,
        success: function(tResult) {
...
function JSnVouchertypeDel(pnPage,ptName,tIDCode) {
            ...
            if (localStorage.StaDeleteArray != '1') {
                ...
                $.ajax({
                    ...
                    success: function(tResult) {
                        ...
                        JSvCallPageVoucherTypeDataTable(pnPage);
                        JCNxCloseLoading();
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        JCNxResponseError(jqXHR, textStatus, errorThrown);
...
//Return : 
//Return Type :
function JSnVouchertypeDelChoose(pnPage) {
    JCNxOpenLoading();
...
function JSvVOCClickPage(ptPage) {
    ...
    JSvCallPageVoucherTypeDataTable(nPageCurrent);
}
...
//Return: View
//Return Type: View
function JSvSearchAllVouchertype() {
    var tSearchAll = $('#oetSearchAll').val();
    JCNxOpenLoading();
    $.ajax({
        type: "POST",
...
function JSbVoucherIsCreatePage(){
    try{
        const tVotCode = $('#oetVotCode').data('is-created');    
        var bStatus = false;
        if(tVotCode == ""){ // No have data
            bStatus = true;
        }
        return bStatus;
    }catch(err){
