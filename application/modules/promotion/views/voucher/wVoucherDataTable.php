            ...
            <table class="table table-striped" style="width:100%">
                <thead>
					<tr class="xCNCenter">
						...
						<?php endif; ?>
						<th class="xCNTextBold"><?= language('promotion/voucher/voucher','tVOCTBCode')?></th>
						<th class="xCNTextBold"><?= language('promotion/voucher/voucher','tVOCTBName')?></th>
                        <th class="xCNTextBold"><?= language('promotion/voucher/voucher','tVOCTBType')?></th>
						<th class="xCNTextBold"><?= language('promotion/voucher/voucher','tVOCTBBarcode')?></th>
                        <th class="xCNTextBold"><?= language('promotion/voucher/voucher','tVOCTBValue')?></th>
						<?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaDelete'] == 1) : ?>
            ...
            <tbody id="odvRGPList">
				<?php if($aDataList['rtCode'] == 1 ):?>
                    <?php foreach($aDataList['raItems'] AS $key=>$aValue){ ?>
                        <tr class="text-center xCNTextDetail2 otrVoucher" id="otrVoucher<?=$key?>" data-code="<?=$aValue['FTVocCode']?>" data-name="<?=$aValue['FTVocName']?>">
							<?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaDelete'] == 1) : ?>
							...
							<?php endif; ?>
							<td><?=$aValue['FTVocCode']?></td>
                            <td><?=$aValue['FTVocName']?></td>
                            <td><?=$aValue['FTVotName']?></td>
                            <td><?=$aValue['FTVocBarCode']?></td>
                            <td><?=$aValue['FCVocValue']?></td>
							<?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaDelete'] == 1) : ?>
								<td><img class="xCNIconTable xCNIconDel" src="<?= base_url().'/application/modules/common/assets/images/icons/delete.png'?>" onClick="JSnVoucherDel('<?=$nCurrentPage?>','<?=$aValue['FTVocName']?>','<?=$aValue['FTVocCode']?>')"></td>
							<?php endif; ?>
							<?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaRead'] == 1) : ?>
								<td><img class="xCNIconTable" src="<?= base_url().'/application/modules/common/assets/images/icons/edit.png'?>" onClick="JSvCallPageVoucherEdit('<?=$aValue['FTVocCode']?>')"></td>
							<?php endif; ?>
                        </tr>
                    <?php } ?>
                <?php else:?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button id="osmConfirm" type="button" class="btn xCNBTNPrimery" onClick="JSnVoucherDelChoose('<?=$nCurrentPage?>')"><?=language('common/main/main', 'tModalConfirm')?></button>
