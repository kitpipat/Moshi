            ...
            <tbody id="odvRGPList">
				<?php if($aDataList['rtCode'] == 1 ):?>
                    <?php foreach($aDataList['raItems'] AS $key=>$aValue){ ?>
                        <tr class="text-center xCNTextDetail2 otrVoucher" id="otrVoucher<?=$key?>" data-code="<?=$aValue['FTVotCode']?>" data-name="<?=$aValue['FTVotName']?>">
							<?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaDelete'] == 1) : ?>
							...
							<?php endif; ?>
							<td><?=$aValue['FTVotCode']?></td>
                            <td><?=$aValue['FTVotName']?></td>
                            <?php if($aValue['FTVotStaUse'] == 1 ) : ?>
								<td><?= language('promotion/voucher/vouchertype','tVOTTBUsing')?></td>
                            <?php else: ?>
                                <td><?= language('promotion/voucher/vouchertype','tVOTTBNoUsing')?></td>
							<?php endif; ?>
							<?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaDelete'] == 1) : ?>
								<td><img class="xCNIconTable xCNIconDel" src="<?= base_url().'/application/modules/common/assets/images/icons/delete.png'?>" onClick="JSnVouchertypeDel('<?=$nCurrentPage?>','<?=$aValue['FTVotName']?>','<?=$aValue['FTVotCode']?>')"></td>
							<?php endif; ?>
							<?php if($aAlwEvent['tAutStaFull'] == 1 || $aAlwEvent['tAutStaRead'] == 1) : ?>
								<td><img class="xCNIconTable" src="<?= base_url().'/application/modules/common/assets/images/icons/edit.png'?>" onClick="JSvCallPageVoucherTypeEdit('<?=$aValue['FTVotCode']?>')"></td>
							<?php endif; ?>
                            </tr>
                            <?php } ?>
                            <?php else:?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button id="osmConfirm" type="button" class="btn xCNBTNPrimery" onClick="JSnVouchertypeDelChoose('<?=$nCurrentPage?>')"><?=language('common/main/main', 'tModalConfirm')?></button>
