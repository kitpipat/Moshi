...
class mWarehouse extends CI_Model {
	...
	public function FSaMWAHSearchByID($paData){
		...
		if(@$tCode){
			$tSQL = "SELECT DISTINCT
						...
						SPNL.FTSpnCode		AS rtSpnCode,
						SPNL.FTSpnName  	AS rtSpnName,
						POSL.FTPosCode		AS rtPosCode,
						POSL.FTPosCode		AS rtPosComName
				FROM [TCNMWaHouse] WAH 
					LEFT JOIN TCNMWaHouse_L WAHL ON WAH.FTWahCode   = WAHL.FTWahCode AND WAH.FTBchCode = WAHL.FTBchCode  AND WAHL.FNLngID = $nLngID
					LEFT JOIN TCNMBranch_L  BCHL ON WAH.FTBchCode = BCHL.FTBchCode  AND BCHL.FNLngID = $nLngID
					LEFT JOIN TCNMShop_L   SHPL ON WAH.FTWahRefCode = SHPL.FTShpCode AND  WAH.FTBchCode = SHPL.FTBchCode  AND SHPL.FNLngID = $nLngID
					LEFT JOIN TCNMSpn_L   SPNL ON WAH.FTWahRefCode = SPNL.FTSpnCode  AND SPNL.FNLngID = $nLngID
					LEFT JOIN TCNMPos POSL ON WAH.FTWahRefCode = POSL.FTPosCode AND WAH.FTBchCode  = POSL.FTBchCode 
				WHERE WAH.FTWahCode = '$tCode' AND WAH.FTBchCode='$tBchCode'  ";
	...
	public function FSnMWAHList($paData){
		...
		$tSQL = "SELECT c.* FROM(
				...
				if(!empty($this->session->userdata('tSesUsrBchCode'))){
					...
					$tSQL .= " AND WAH.FTBchCode ='$tBchCode' ";
			   }
	...
	function JSnMWAHGetPageAll($ptSearchList,$ptLngID){	
		$tSQL = "SELECT COUNT (WAH.FTWahCode) AS counts
		    					FROM TCNMWaHouse WAH
								LEFT JOIN TCNMWaHouse_L WAHL ON WAH.FTWahCode = WAHL.FTWahCode AND WAH.FTBchCode = WAHL.FTBchCode AND WAHL.FNLngID = $ptLngID
								LEFT JOIN TCNMBranch_L BCHL ON  WAH.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID = $ptLngID
								WHERE 1 = 1";
