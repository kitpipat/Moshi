...
class cWarehouse extends MX_Controller {
    ...
    public function index($nBrowseType,$tBrowseOption){
		...
		$this->load->view ( 'company/warehouse/wWarehouse', array ('vBtnSave'			=>$vBtnSave,
																'nBrowseType'		=>$nBrowseType,
    ...
    public function FSaCWAHEditEvent(){
		try{
			...
			switch($FTWahStaType){
				case '1':
					$FTWahRefCode	= $this->input->post('oetWAHBchCode');
				break;
				case '2':
					$FTWahRefCode	= $this->input->post('oetWAHBchCode');
				break;
				...
				break;
				case '5':
					$FTWahRefCode	= $this->input->post('oetSpnCode');
				break;
				case '6':
					$FTWahRefCode	= $this->input->post('oetWahPosCode');
				break;

				default:
					$FTWahRefCode	= $this->input->post('oetWahRefCode');
			}
			
			$aDataMaster = array(
					'FTWahCode'     => $this->input->post('oetWahCode'),
					'FTWahName'     => $this->input->post('oetWahName'),
					'FTWahStaType'  => $this->input->post('ocmWahStaType'),
					'FTBchCode'     => $this->input->post('oetWAHBchCode'),
					'FTBchCodeOld'  => $this->input->post('oetWAHBchCodeOld'),
					'FTWahRefCode'  => $FTWahRefCode,
    ...
    public function FSaCWAHAddEvent(){
		try{
			...
			if($tIsAutoGenCode == '1'){
				...
				$aGenCode	= FCNaHGenCodeV5('TCNMWaHouse','0',$this->input->post('oetWAHBchCode'));
			...
			switch($FTWahStaType){
				...
				break;
				case '5':
					$tWahRefCode	= $this->input->post('oetSpnCode');
				break;
				case '6':
					$tWahRefCode	= $this->input->post('oetWahPosCode');
				break;
			...
			$this->db->trans_begin();
			$aResAdd    = $this->mWarehouse->FSaMWAHAdd($aDataMaster);
		...
		if(is_array($aResList) && $aResList['rtCode'] == '1' ){
			...
			if($aResList['roItem']['rtWahStaType'] == '1' || '2'){
				...
				$vWahStaType = '<option value="1" disabled>'. language('company/warehouse/warehouse', 'tWahStaTypeSEL1') .'</option>
								<option value="2" disabled>'. language('company/warehouse/warehouse', 'tWahStaTypeSEL2') .'</option>
				...
				$vWahStaType = '<option value="1" disabled>'. language('company/warehouse/warehouse', 'tWahStaTypeSEL1') .'</option>
								<option value="2" disabled>'. language('company/warehouse/warehouse', 'tWahStaTypeSEL2') .'</option>
								<option value="3" disabled>'. language('company/warehouse/warehouse', 'tWahStaTypeSEL3') .'</option>
								<option value="4" >'. language('company/warehouse/warehouse', 'tWahStaTypeSEL4') .'</option>
								<option value="5" >'. language('company/warehouse/warehouse', 'tWahStaTypeSEL5') .'</option>
								<option value="6" disabled>'. language('company/warehouse/warehouse', 'tWahStaTypeSEL6') .'</option>';
			}
