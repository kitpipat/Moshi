...
class mCustomer extends CI_Model {
    ...
    public function FSaMCSTList($ptAPIReq, $ptMethodReq, $paData){
        ...
        $nLngID     = $paData['FNLngID'];
        $tSQL     = "SELECT c.* FROM(
                    SELECT  ROW_NUMBER() OVER(ORDER BY rtCreateOn DESC, rtCstCode DESC) AS rtRowID,*
                        FROM
                        (SELECT DISTINCT
                            CST.FTCstCode AS rtCstCode,
                            CST.FTCstEmail AS rtCstEmail,
    ...
    public function FSaMCSTAddUpdateMaster($paData){
        ...
        try{
            ...
            $this->db->set('FTCstFax', $paData['FTCstFax']);
  ...
  /**
    ...
    public function FSaMCSTGetAddress4MQ($ptCstCode){
      ...
      /**
     * Functionality : FSaMCSTGetCstLevLang4MQ
     * Parameters : $paData
     * Creator : 01/04/2020 Nale
     * Last Modified : -
     * Return : status
     * Return Type : array
     */
    public function FSaMCSTGetCstLevLang4MQ($ptClvCode,$ptCgpCode){
     
        $tSqlCstLev="  SELECT '$ptCgpCode' AS FTCgpCode,
        CSTL.FTClvCode AS FTLevCode,
        CSTL.FNLngID AS FNLngID,
        CSTL.FTClvName AS FTLevName,
        CSTL.FTClvRmk AS FTLevRmk
        FROM TCNMCstLev_L CSTL WITH (NOLOCK)
        WHERE CSTL.FTClvCode='$ptClvCode' ";
        $oQuery = $this->db->query($tSqlCstLev);
        $aCstLevMaster_L = $oQuery->result_array();

        return $aCstLevMaster_L;
    }
    ...
    public function FScMCSTGetAmtActive($ptCstCode){
      $tCgpCode   = $this->db->where('FTSysCode','AMQMember')->where('FTSysSeq',1)->get('TSysConfig')->row_array()['FTSysStaUsrValue'];
      $aTxnAmyActive = $this->db->where('FTCgpCode',$tCgpCode)->where('FTMemCode',$ptCstCode)->get('TCNTMemAmtActive')->row_array();
      return $aTxnAmyActive;
    }
