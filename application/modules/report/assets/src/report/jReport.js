    ...
    if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
        ...
        $.ajax({
            ...
            success: function(tResult) {
                ...
                // console.log('[JSvCallPageReportMain] -> FCNoCRPTViewPageMain : ' + aReturnData['nDiffTime'] + 's');
                ...
                console.log('[JSvCallPageReportMain] -> ajaxSuccess : ' + seconds + 's');
            },
...
function JSvCallPageReportCondition() {
    ...
    $.ajax({
        ...
        success: function(tResult) {
            var aReturnData = JSON.parse(tResult);
            // console.log('[JSvCallPageReportCondition] -> FCNoCRPTViewCondition : ' + aReturnData['nDiffTime'] + 's');
            if (aReturnData['nStaEvent'] == '1') {
            ...
            var endDate   = new Date();
            var seconds = (endDate.getTime() - startDate.getTime()) / 1000;
            console.log('[JSvCallPageReportCondition] -> ajaxSuccess : ' + seconds + 's');
        },
        error: function(jqXHR, textStatus, errorThrown) {
            JCNxResponseError(jqXHR, textStatus, errorThrown);
        }
...
function JCNxRptChkDataInHisExport() {
    ...
    $.ajax({
        ...
        success: function(tResult) {
            ...
            // console.log('[JCNxRptChkDataInHisExport] -> FCNoCRPTChkDataInTSysHisExport : ' + aDataReturn['nDiffTime'] + 's');
            ...
            var endDate   = new Date();
            var seconds = (endDate.getTime() - startDate.getTime()) / 1000;
            console.log('[JCNxRptChkDataInHisExport] -> ajaxSuccess : ' + seconds + 's');
        },
        error: function(jqXHR, textStatus, errorThrown) {
            JCNxResponseError(jqXHR, textStatus, errorThrown);
        }
...
function JCNxRptChkConditionExport(paDataReturn) {
    ...
    var tSysBchCode = paDataReturn['tSysBchCode'];
    if (aDataHisExport != "") {
        if (aDataHisExport['FTStaDownload'] == '0' && aDataHisExport['FTStaZip'] == '0') {
            ...
            // Prefix Queue Name
            let tQueueNameSubscribe = 'RESRPT_' + tSysBchCode + '_' + tRptCode + '_' + tRptUserCode + '_' + tRptUserSessionID + '_' + dDateSubscribe + '_' + dTimeSubscribe;
