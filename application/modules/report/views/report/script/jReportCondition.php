...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        if(tUsrBchCode  != ""){ 
            ...
            $('#obtRptMultiBrowseBranch').attr("disabled", true);
        }
    ...
    var oRptBrowseCashierFrom = function(poCashierReturnInput){
        ...
        let oOptionReturnCashierFrom    = {
            Title: ['authen/user/user', 'tUsrCashier'],
    ...
    // Last Update By : Napat(Jame) 21/10/2020 เพิ่ม where เงื่อนไข FTBchCode , FTMerCode และเปลี่ยน Lang 
    // Option bbk Option
    var oRptBrowseBBk = function(BbkReturnInput){
        ...
        let tRptBchCode             = BbkReturnInput.tRptBchCode;
        let tRptMerCode             = BbkReturnInput.tRptMerCode;
        let tRptBBkWhere            = '';

        let tRptBchCodeReplace      = tRptBchCode.replace(",", "','");
        let tRptMerCodeReplace      = tRptBchCode.replace(",", "','");

        if( tRptBchCode != '' ){
            tRptBBkWhere += " AND TFNMBookBank.FTBchCode IN ('" + tRptBchCodeReplace + "') ";
        }
        if( tRptMerCode != '' ){
            tRptBBkWhere += " AND TFNMBookBank.FTMerCode IN ('" + tRptMerCodeReplace + "') ";
        }

        let oOptionReturnBbk    = {
            Title: ['BookBank/BookBank/BookBank', 'tBBKTitle'],
            Table: {Master: 'TFNMBookBank', PK: 'FTBbkAccNo'},
            Join: {
                Table   : ['TFNMBookBank_L','TCNMBranch_L'],
                On      : [
                    'TFNMBookBank_L.FTBbkCode = TFNMBookBank.FTBbkCode AND TFNMBookBank.FTBchCode = TFNMBookBank_L.FTBchCode AND TFNMBookBank_L.FNLngID = ' + nLangEdits,
                    'TFNMBookBank.FTBchCode = TCNMBranch_L.FTBchCode AND TCNMBranch_L.FNLngID = ' + nLangEdits
                ]
            },
            Where: {
                Condition : [ tRptBBkWhere ]
            },
            GrideView       : {
                ColumnPathLang  : 'BookBank/BookBank/BookBank', 
                ColumnKeyLang   : ['tBBKTablebanch','tBBKTableBbkCode', 'tBBKTableIDCode'],
                ColumnsSize     : ['15%','15%', '70%'],
                WidthModal      : 50,
                DataColumns     : ['TCNMBranch_L.FTBchName','TFNMBookBank.FTBbkCode', 'TFNMBookBank.FTBbkAccNo'],
                DataColumnsFormat: ['', '', ''],
                Perpage: 5,
            ...
            CallBack:{
                ReturnType	: 'S',
                Value		: [tBbkInputReturnBbkCode,"TFNMBookBank.FTBbkAccNo"],
                Text		: [tBbkInputReturnBbkName,"TFNMBookBank.FTBbkAccNo"],
            },
            NextFunc : {
    ...
    $('#obtRptMultiBrowsePos').unbind().click(function(){
        ...
        if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
            ...
            // if(tDataShop != '' && tDataShop !=  undefined){
            ...
            oPosBrowseMultiOption           = {
                ...
                Where :{
                    Condition : ['AND (TCNMPos.FTPosType IN (1,2,3,4)) '+tTextWhereInBranch+tTextWhereInShop]
                },
                GrideView   : {
                    ColumnPathLang      : 'pos/salemachine/salemachine',
    ...
    $('#obtBrowseCashierTo').unbind().click(function(){
       ...
       // Browse Event Shop
       $('#obtRPTBrowseBookBankFrom').unbind().click(function(){
        var nStaSession = JCNxFuncChkSessionExpired();
        if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
            JSxCheckPinMenuClose(); // Hidden Pin Menu
            // let tRptModCode     = $('#ohdRptModCode').val();
            ...
            oOptionReturnBbk          = oRptBrowseBBk({
                'tReturnInputBbkCode'  : 'oetRptBbkAccNoFrom', //tReturnInputBbkCode
                'tReturnInputBbkName'  : 'oetRptBbkAccNameFrom', //tReturnInputBbkName
                'tRptBchCode'          : $('#oetRptBchCodeSelect').val(),
                'tRptMerCode'          : $('#oetRptMerCodeSelect').val(),
                // 'tRptModCode'       : tRptModCode,
    ...
    $('#obtRPTBrowseBookBankTo').unbind().click(function(){
        var nStaSession = JCNxFuncChkSessionExpired();
        if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
            JSxCheckPinMenuClose(); // Hidden Pin Menu
            ...
            oOptionReturnBbkTo        = oRptBrowseBBk({
                'tReturnInputBbkCode'  : 'oetRptBbkAccNoTo',
                'tReturnInputBbkName'  : 'oetRptBbkAccNameTo',
                'tRptBchCode'          : $('#oetRptBchCodeSelect').val(),
                'tRptMerCode'          : $('#oetRptMerCodeSelect').val(),
    ...
    function JSxRptConsNextFuncBrowseBbkFrom(poDataNextFunc){
        if(typeof(poDataNextFunc) != 'undefined' && poDataNextFunc != "NULL"){
            ...
            var tBbkCode = aDataNextFunc[0];
            var tBbkAccNo = aDataNextFunc[1];
            // $('#oetRptBbkAccNoFrom').val(tBbkAccNo);
            // $('#oetRptBbkAccNameFrom').val(tBbkAccNo);

            if( $('#oetRptBbkAccNoTo').val() == '' ){
                $('#oetRptBbkAccNoTo').val(tBbkAccNo);
                $('#oetRptBbkAccNameTo').val(tBbkAccNo);
            }
        }else{
            $('#oetRptBbkAccNoFrom').val('');
            $('#oetRptBbkAccNameFrom').val('');
        }
    }
    ...
    function JSxRptConsNextFuncBrowseBbkTo(poDataNextFunc){
        if(typeof(poDataNextFunc) != 'undefined' && poDataNextFunc != "NULL"){
            ...
            if( $('#oetRptBbkAccNoFrom').val() == '' ){
                $('#oetRptBbkAccNoFrom').val(tBbkAccNo);
                $('#oetRptBbkAccNameFrom').val(tBbkAccNo);
            }
        }else{
            $('#oetRptBbkAccNoTo').val('');
            $('#oetRptBbkAccNameTo').val('');
        }
    }
