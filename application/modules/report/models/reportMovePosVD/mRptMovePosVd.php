...
class mRptMovePosVd extends CI_Model
{
    ...
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        $tSQL   = " SELECT
                    ...
                    FROM (
                        SELECT DISTINCT
                            ROW_NUMBER() OVER(ORDER BY DATASTKCRD.FTBchCode ASC,DATASTKCRD.FTWahCode ASC,DATASTKCRD.FTPdtCode ASC, DATASTKCRD.FTRptRowSeq ASC) AS RowID,
                            DATASTKCRD.*,
                            DATASUBPDT.*,
                            DATASUBWAH.*,
                            DATASUBBCH.*
        ...
        $tSQL   .=  "   ORDER BY L.FTBchCode,L.FTWahCode ,L.FTPdtCode,L.FTRptRowSeq ,L.FNRowPartID";
    ...
    public function FMxMRPTSetPriorityGroup($ptComName, $ptRptCode, $ptUsrSession)
    {
        $tSQLUPD   = "  UPDATE DATAUPD
                        ...
                        RIGHT JOIN (
                            SELECT 
                                ROW_NUMBER() OVER(PARTITION BY FTBchCode,FTWahCode,FTPdtCode ORDER BY FTBchCode ASC,FTWahCode ASC,FTPdtCode ASC,FTRptRowSeq ASC) AS PartIDPdt,
    ...
    private function FMxMRPTAjdStkBal($ptComName, $ptRptCode, $ptUsrSession)
    {
        ...
        $tSQL   = " UPDATE STK
                    ...
                    LEFT JOIN (
                        ...
                        FROM (
                            SELECT
                                FTRptRowSeq, FTWahCode,FTPdtCode,FTStkDocNo,
