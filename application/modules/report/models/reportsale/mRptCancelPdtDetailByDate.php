...
class mRptCancelPdtDetailByDate extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{CALL SP_RPTxCancelDTByDate(?,?,?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            'pnLngID' => $nLangID, 
            'pnComName' => $tComName,
            'ptRptCode' => $tRptCode,
    ...
    */
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " 
                ...
                FROM TRPTCancelDTByDateTmp   WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL = "   
            ...
            FROM (
                SELECT
                    ...
                    A.*,
                    S.FNRptGroupMember
                FROM TRPTCancelDTByDateTmp A WITH(NOLOCK)
                /* Calculate Misures */
                LEFT JOIN (
                    SELECT
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   
            ...
            FROM TRPTCancelDTByDateTmp TSBP WITH(NOLOCK)
            WHERE 1=1
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere)
    {
        ...
        $tSQL = "
            UPDATE TRPTCancelDTByDateTmp
                SET TRPTCancelDTByDateTmp.FNRowPartID = B.PartID
            FROM (
                SELECT
            ...
            WHERE 1=1
            AND TRPTCancelDTByDateTmp.FTRptRowSeq = B.FTRptRowSeq
            AND TRPTCancelDTByDateTmp.FTComName = '$tComName' 
            AND TRPTCancelDTByDateTmp.FTRptCode = '$tRptCode'
            AND TRPTCancelDTByDateTmp.FTUsrSession = '$tUsrSession'
        ";
        $this->db->query($tSQL);
    }
    ...
    public function FSnMCountRowInTemp($paParams)
    {
        ...
        $tSQL = "   
            SELECT
                TMP.FTRptCode
            FROM TRPTCancelDTByDateTmp TMP WITH(NOLOCK)
