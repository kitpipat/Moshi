...
class mRptSaleByProduct extends CI_Model {
    ...
    public function FSaMGetDataReport($paDataWhere) {
                        ...
                        FROM (
                            SELECT RESULT.* ,
                                    ...
                                    FROM( 
                                        ...
                                        ROW_NUMBER() OVER(PARTITION BY FTBchCode ORDER BY FTBchCode DESC) AS FNRowPartID1,
                                        ROW_NUMBER() OVER(PARTITION BY FTBchCode , FDXshDocDate ORDER BY FTBchCode , FDXshDocDate , FNRowPartID) AS FNRowPartDocDate,
                                            A.* 
        ...
        $tSQL .= "
                                GROUP BY FTBchCode
                                ORDER BY FTBchCode OFFSET 0 ROWS
                            ) AS S ON RESULT.FTBchCode = S.FTBchCode_SUM
                            WHERE 1=1
                            AND RESULT.FTComName     = '$tComName'
                            AND RESULT.FTRptCode     = '$tRptCode'
                            AND RESULT.FTUsrSession  = '$tUsrSession'";
        if($tAppType != ""){
            $tSQL .= "      AND RESULT.FNAppType       = '$tAppType'";
        }
                            /* End Calculate Misures */
        $tSQL .= "      ORDER BY RESULT.FTBCHCode , RESULT.FNRowPartID OFFSET 0 ROWS ) AS L
        

                        LEFT JOIN ( SELECT COUNT(FDXshDocDate) AS FDXshDocDateCount, 
                                FTUsrSession AS FTUsrSession_SumDocDate, 
                                FDXshDocDate AS FDXshDocDate_SumDocDate, 
                                SUM(ISNULL(FCXsdQty, 0)) AS FCXsdQty_SumDocDate, 
                                SUM(ISNULL(FCXsdAmtB4DisChg, 0)) AS FCXsdAmtB4DisChg_SumDocDate, 
                                SUM(ISNULL(FCXsdDis, 0)) AS FCXsdDis_SumDocDate, 
                                SUM(ISNULL(FCXsdSetPrice, 0)) AS FCXsdSetPrice_SumDocDate,
                                SUM(ISNULL(FCXsdNetAfHD, 0)) AS FCXsdNetAfHD_SumDocDate,
                                FTBchCode
                         FROM TRPTSalDTTmp WITH(NOLOCK)
                            WHERE 1=1 
                            AND FTComName       = '$tComName' 
                        ...
                        $tSQL .= "
                            GROUP BY FTUsrSession ,FDXshDocDate,FTBchCode
                            ORDER BY FTBchCode OFFSET 0 ROWS
                        ) D ON L.FTUsrSession = D.FTUsrSession_SumDocDate AND L.FDXshDocDate = D.FDXshDocDate_SumDocDate AND L.FTBchCode = D.FTBchCode
