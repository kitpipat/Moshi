...
class mRptSalePaymentSummary extends CI_Model {
   ...
   //Functionality: Delete Temp Report
        ...
        $tSQL = " SELECT c. * FROM(
            SELECT  ROW_NUMBER() OVER(ORDER BY rtRptCode ASC) AS rtRowID,* FROM (
                SELECT TOP 100 PERCENT
                    ...
                    FCXrcNet       AS rtXrcNet,
                    FTUsrSession   AS rtUsrSession,
                    FTComName      AS rtComname,
                    FTRptCode      AS rtRptCode
            FROM TRPTVDSalRCTmp 
            WHERE 1 = 1
                AND FTUsrSession = '$tUserSessionID' AND FTComName = '$tCompName' AND FTRptCode = '$tRptCode'
                ORDER BY 
                rtRptCode ASC
    ...
    public function FSaMGetDataSumFootReport($paDataWhere,$paDataFilter){
        ...
        $tSQL   =   "   SELECT
                            ISNULL(SUM(FCXrcNet),0) 	AS FCXrcNet_SumFooter
                        FROM TRPTVDSalRCTmp
                        WHERE 1 = 1 
                        AND FTComName = '".$tCompName."' AND FTRptCode = '".$tRptCode."' AND FTUsrSession = '".$tUserSessionID."'
                    ";
