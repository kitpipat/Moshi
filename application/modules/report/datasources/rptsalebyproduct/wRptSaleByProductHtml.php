        ...
        <div class="xCNContentReport">
            <div id="odvRptTableAdvance" class="table-responsive">
                <table class="table">
                    <thead>
                        <tr>
                            <th nowrap class="text-left xCNRptColumnHeader" style="width:7%;text-indent: 45px;" ><?php  echo $aDataTextRef['tRptPdtCode']; ?></th>
                        ...
                        <?php if(isset($aDataReport['aRptData']) && !empty($aDataReport['aRptData'])):?>
                            ...
                            <?php foreach($aDataReport['aRptData'] as $nKey=>$aValue):?>
                                <?php
                                    ...
                                    $nRowPartID     = $aValue["FNRowPartID1"]; 
                                ?>
                                <?php
                                    ...
                                    //Step 2 Groupping data
                                    $aGrouppingDataDocDate = array(date("d/m/Y", strtotime($aValue['FDXshDocDate'])));
                                    $nRowPartDocDate     = $aValue["FNRowPartDocDate"]; 
                                    // FCNtHRPTHeadGrouppingRptTSPBch($nRowPartDocDate,$aGrouppingDataDocDate);

                                    if($nRowPartDocDate == '1'){
                                        echo '<tr>';
                                        for($i = 0;$i<count($aGrouppingDataDocDate);$i++){
                                            if($aGrouppingDataDocDate[$i] == $aGrouppingDataDocDate[0] ){
                                                echo "<td class='xCNRptGrouPing  text-left' colspan='6' style='padding: 5px;text-indent: 30px;'>".language('report/report/report', 'tRptSRCDate').' : '.$aGrouppingDataDocDate[$i]."</td>";
                                            }else{
                                                echo "<td class='xCNRptGrouPing text-right'  style='padding: 5px;'>".$aGrouppingDataDocDate[$i]."</td>";
                                            }
                                        }
                                        echo '</tr>';
                                    }
                                ...
                                <tr>
                                    <td nowrap class="text-left xCNRptDetail" style="text-indent: 45px;"><?php echo '&nbsp;&nbsp;'.$aValue["FTPdtCode"];?></td>
                                    ...
                                    <td nowrap class="text-right xCNRptDetail"><?php echo number_format($aValue["FCXsdAmtB4DisChg"], $nOptDecimalShow);?></td>
                                    ...
                                    <td nowrap class="text-right xCNRptDetail"><?php echo number_format($aValue["FCXsdNetAfHD"], $nOptDecimalShow);?></td>
                                </tr>

                                <?php
                                    $nGroupMember1   = $aValue["FDXshDocDateCount"];

                                    $aSumFooter1     = array($aDataTextRef['tRptTotal'].' '.language('report/report/report', 'tRptSRCDate').' : '.date("d/m/Y", strtotime($aValue['FDXshDocDate'])),number_format(@$aValue['FCXsdQty_SumDocDate'], $nOptDecimalShow),'N',number_format(@$aValue['FCXsdAmtB4DisChg_SumDocDate'], $nOptDecimalShow),number_format(@$aValue['FCXsdDis_SumDocDate'], $nOptDecimalShow),'N',number_format(@$aValue['FCXsdNetAfHD_SumDocDate'], $nOptDecimalShow));

                                        if ($nRowPartDocDate == $nGroupMember1) {
                                            ...
                                            for ($i = 0; $i < count($aSumFooter1); $i++) {
                                                ...
                                                echo '<td  '.$tColSpan.' class="xCNRptSubFooter" style="' . $tStyle . ' ;padding: 4px;text-indent: 30px;">' . $tFooterVal . '</td>';
                                            }
                                            echo '</tr>';
                                        }
                                    ...
                                    $tSumBranch             = $aDataTextRef['tRptTotal'].' '.$aDataTextRef['tRptBranch'].' '.$aValue["FTBchCode"].' '.$aValue["FTBchName"];
