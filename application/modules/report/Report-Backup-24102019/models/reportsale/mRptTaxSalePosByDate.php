...
class mRptTaxSalePosByDate extends CI_Model {
    ...
    private function FMxMRPTSetPriorityGroup($paDataWhere){
        ...
        $tSQL = "   
            UPDATE TRPTPSTaxHDTmp
                SET TRPTPSTaxHDTmp.FNRowPartID = B.PartID
                FROM (
                    SELECT
                        ROW_NUMBER() OVER(PARTITION BY TMP.FDXshDocDate ORDER BY TMP.FDXshDocDate DESC) AS PartID ,
                        TMP.FTRptRowSeq
                    FROM TRPTPSTaxHDTmp TMP WITH(NOLOCK)
    ...
    public function FSaMGetDataReport($paDataWhere){
        ...
        $tSQL   =   "   
            ...
            FROM (
                SELECT
                    ROW_NUMBER() OVER(ORDER BY FDXshDocDate DESC) AS RowID,
                    A.*,
                    S.FNRptGroupMember
        ...
        $tSQL   .=  " ORDER BY L.FDXshDocDate DESC";
    ...
    public function FSnMExecStoreReport($paParams){
        ...
        $tReciveTypeCodeFrom = empty($paParams['oetRptRcvCodeFrom']) ? '' : $paParams['oetRptRcvCodeFrom']; 
        $tReciveTypeCodeTo = empty($paParams['oetRptRcvCodeTo']) ? '' : $paParams['oetRptRcvCodeTo']; 
        */
        // วันที่
        ...
        $tCallStore = "{CALL SP_RPTxPSSVatByDate1001007(?,?,?,?,?,?,?,?,?)}";
        $aDataStore = array(
            'pnLngID' => $nLangID , 
            'pnComName' => $tComName,
            'ptRptCode' => $tRptCode,
