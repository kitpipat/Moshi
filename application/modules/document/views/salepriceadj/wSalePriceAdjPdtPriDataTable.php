        ...
        <div class="table-responsive">
            <table id="otbSpaDataList" class="table table-striped" style="margin-bottom: 0px;">
                <thead>
                    ...
                    <th><?= language('document/salepriceadj/salepriceadj','tPdtPriTBDelete')?></th>
                ...
                <tbody>
                    <?php if($aPdtPriDataList['rtCode'] == 1 ):?>
                        <?php foreach($aPdtPriDataList['raItems'] as $DataTableKey=>$DataTableVal){?>
                            <tr class="text-center xCNTextDetail2 otrSpaPdtPri" id="otrSpaPdtPri<?=$DataTableVal['FNXtdSeqNo']?>" name="otrSpaPdtPri" data-doc="<?=$DataTableVal['FTXthDocNo']?>" data-code="<?=$DataTableVal['FTPdtCode']?>" data-pun="<?=$DataTableVal['FTPunCode']?>" data-seq="<?=$DataTableVal['FNXtdSeqNo']?>" data-page="<?=$nCurrentPage?>">
                                ...
                                <?php if($DataVal->FTShwStaAlwEdit == 1){ ?>
                                        ...
                                        <div class="xCNHide xWEditInLine<?=$DataTableVal['FNXtdSeqNo']?>">
                                            <input 
                                                type="<?=$InputType?>" 
                                                class="form-control xWValueEditInLine<?=$DataTableVal['FNXtdSeqNo']?> <?=$tValidateType?> <?=$tAlignFormat;?>"
                                ...
                                <?php }else{   ?>
                                    <label class="xCNPdtFont xWShowInLine xWShowValue<?=$tColumnName?><?=$DataTableVal['FNXtdSeqNo']?>"><?=$tDataCol?></label>
                        ...
                        <?php } ?>
                            <tr>
                                <td nowrap class="text-right" colspan="<?php echo count($aColumnShow) - count($aDataSeqCollumSumFooter)+1; ?>"><?= language('document/salepriceadj/salepriceadj','tPdtPriSumTotalPrice')?></td>
                            <?php  if(!empty($aDataSeqCollumSumFooter)){
                                foreach($aDataSeqCollumSumFooter as $tSeqColName){ ?>
                                <td nowrap class="text-right"><?=number_format($aPdtPriDataList['rnSumTotalPrice'][$tSeqColName], $nOptDecimalShow);?></td>
                            <?php   } } ?>
                                <!-- <td nowrap class="text-right"><?=number_format($aPdtPriDataList['rnSumTotalPrice']['FCXtdPriceWhs'], $nOptDecimalShow);?></td>
                                <td nowrap class="text-right"><?=number_format($aPdtPriDataList['rnSumTotalPrice']['FCXtdPriceNet'], $nOptDecimalShow);?></td> -->
                                <td nowrap ></td>
                            </tr>
                    <?php else:?>
                        <tr><td class='text-center xCNTextDetail2 xWTextNotfoundDataSalePriceAdj' colspan='99'><?= language('common/main/main','tCMNNotFoundData')?></td></tr>
                    <?php endif;?>
                </tbody>
            </table>
...
$('.xCNDeleteInLineClick').on('click',function(){
    ...
    JSoSpaPdtPriDel(nPage,tDoc,tPdt,tPun);
});
...
$('ducument').ready(function(){
	...
	$('.ocbListItem').click(function(){
        ...
        if(aArrayConvert == '' || aArrayConvert == null){
            obj.push({"tSeq": tSeq, "tPdt": tPdt, "tDoc": tDoc, "tPun": tPun });
