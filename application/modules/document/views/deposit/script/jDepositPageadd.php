<script>
    ...
    $('#obtBrowseDPSReason').click(function(){ JCNxBrowseData('oBrowseDPSReason'); });
          //เหตุผล
          var oBrowseDPSReason = {
            ...
            CallBack: {
                ReturnType	: 'S',
                Value		: ['oetDPSReasonCode',"TCNMRsn.FTRsnCode"],
                Text		: ['oetDPSReasonName',"TCNMRsn_L.FTRsnName"]
            }
        }
    /*===== Begin Event Browse =========================================================*/
    // เลือกสาขา
    ...
    $("#obtDepositBrowseBookCheque").click(function() {
        ...
        window.oDepositBrowseBookCheque = {
            ...
            Join: {
                ...
                On: [
                    'TFNMBookCheque_L.FTChqCode = TFNMBookCheque.FTChqCode AND TFNMBookCheque_L.FTBchCode = TFNMBookCheque.FTBchCode AND TFNMBookCheque_L.FNLngID = ' + nLangEdits, 
            ...
            Where: {
                Condition: [
                    function() {
                        ...
                        var tSQL = " AND TFNMBookCheque.FTBchCode = '" + $("#oetDepositBchCode").val() + "'";
                        return tSQL;
                    }
                ]
            },
            GrideView: {
                ...
                ColumnKeyLang: ['tBcqtCode', 'tBcqtBBName'],
                ColumnsSize: ['15%', '75%'],
                DisabledColumns: [2],
                WidthModal: 50,
    ...
    function JSxDepositValidateForm() {
        var oTopUpVendingForm = $('#ofmDepositForm').validate({
            ...
            submitHandler: function(form) {
                var cDepositCashRefAmt = 0;
                $('.xCNDepositCashRefAmt').each(function(){
                    if($(this).val()!=''){
                        cDepositCashRefAmt = cDepositCashRefAmt + parseFloat($(this).val());
                    }
                });
                if(cDepositCashRefAmt>0){

                    if($('#ohdDepositStaDelMQ').val()==''){
                    $('#ohdDepositStaDelMQ').val(1);
                    JSxDepositSave();
                    }
                }else{

                FSvCMNSetMsgWarningDialog('<?=language('document/deposit/deposit','tDplValidateAmout')?>');
                }
    ...
    function JSvDepositCancel(pbIsConfirm) {
        ...
        if (typeof nStaSession !== "undefined" && nStaSession == 1) {
            ...
            if($('#oetDPSReasonCode').val()!=''){
                var tDPSReasonCode = $('#oetDPSReasonCode').val();
                $.ajax({
                    type: "POST",
                    url: "depositDocCancel",
                    data: {
                        tDocNo: tDocNo,
                        tDPSReasonCode:tDPSReasonCode
                    },
                    cache: false,
                    timeout: 5000,
                ...
                FSvCMNSetMsgWarningDialog('<?= language('document/deposit/deposit', 'tDesRsnCanCel'); ?>');
            }
            } else {
