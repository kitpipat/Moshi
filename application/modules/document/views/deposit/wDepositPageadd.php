...
if ($aResult['rtCode'] == "1") { // Edit
	...
	$tSenderCode = $aResult['raItems']['tSenderCode'];
	$tSenderName = $aResult['raItems']['tSenderName'];
	$dLstUpdateOn = date('Y-m-d',strtotime($aResult['raItems']['FDLastUpdOn']));
	...
	$tUserShpCode = "";
	$tUserShpName = "";
	$tSenderCode = $this->session->userdata('tSesUsername');
	$tSenderName = $this->session->userdata('tSesUsrUsername');
	$dLstUpdateOn = date('Y-m-d');
...
if(($dLstUpdateOn==date('Y-m-d') && $bIsApv && !$bIsCanCel) || (!$bIsApv && !$bIsCanCel)){
	$nDepositStaCancel = 1;
}else{
	$nDepositStaCancel = 0;
}
...
<form class="contact100-form validate-form" action="javascript:void(0)" method="post" enctype="multipart/form-data" autocorrect="off" autocapitalize="off" autocomplete="off" id="ofmDepositForm">
	...
	<input type="hidden" name="nDepositStaCancel"  id="nDepositStaCancel" value="<?=$nDepositStaCancel?>">
	<div class="row">
		<!--Panel เงื่อนไข-->
		<div class="col-md-3">
			<!--Section : เงื่อนไข-->
					...
					<div class="panel-body xCNPDModlue">
						...
						<?php if ($tUserLoginLevel == 'HQ' && false) { ?>
							<div class="form-group">
								...
								<div class="input-group">
									<input name="oetTopUpVendingBCHName" id="oetTopUpVendingBCHName" class="form-control" value="" type="text" readonly="" placeholder="<?= language('document/deposit/deposit', 'tTopUpVendingBCH') ?>" data-validate-required="<?= language('document/deposit/deposit', 'tTopUpVendingBCHValidate') ?>">
									<input name="oetDepositBchCode" id="oetDepositBchCode" value="" class="form-control xCNHide" type="text">
									<span class="input-group-btn">
										<button class="btn xCNBtnBrowseAddOn" id="obtDepositBrowseBch" type="button">
						...
						<?php } else { ?>
							<input name="oetDepositBchCode" id="oetDepositBchCode" value="<?php echo $tUserBchCode; ?>" class="form-control xCNHide" type="text">
							<label class="xCNLabelFrm"><?= $tUserBchName ?></label>
							<div>
								<hr>
							</div>
						<?php } ?>
						...
						<div class="form-group">
							...
							<div class="input-group">
								...
								class="form-control" 
								value="<?php echo $tSenderName; ?>" 
								type="text" 
								readonly 
								placeholder="<?= language('document/producttransferwahouse/producttransferwahouse', 'tDepositor') ?>" 
								data-validate-required="<?= language('document/deposit/deposit', 'tTopUpVendingMerValidate') ?>">
								<input name="oetDepositUsrCode" id="oetDepositUsrCode" value="<?php echo $tSenderCode; ?>" class="form-control xCNHide" type="text">
								<span class="input-group-btn">
...
<?php if(!$bIsApvOrCanCel || ($nDepositStaCancel==1)) { ?>
	<!-- Begin Approve Doc -->
	<div class="modal fade" id="odvDepositPopupApv">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<p><strong><?php echo language('document/document/document', 'tDocCancelText2') ?></strong></p>
					                        <!--เลือกเหตุผล-->
					<div class="form-group">
                            <label class="xCNLabelFrm"><?= language('document/deposit/deposit', 'tDesRsnCanCel'); ?></label>
                            <div class="input-group">
                                <input name="oetDPSReasonName" id="oetDPSReasonName" class="form-control" value="" type="text" readonly="" 
                                        placeholder="<?= language('document/transferwarehouseout/transferwarehouseout', 'tTWOReason') ?>">
                                <input name="oetDPSReasonCode" id="oetDPSReasonCode" value="" class="form-control xCNHide" type="text">
                                <span class="input-group-btn">
                                    <button class="btn xCNBtnBrowseAddOn xCNApvOrCanCelDisabled" id="obtBrowseDPSReason" type="button">
                                        <img src="<?= base_url() . '/application/modules/common/assets/images/icons/find-24.png' ?>">
                                    </button>
                                </span>
                            </div>
					...
					<div class="row">
						...
						<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
							<div class="form-group">
								...
								<input type="text" class="form-control xCNInputWithoutSingleQuote xCNInputNumericWithDecimal   text-right" id="oetDepositCashAddValue" name="oetDepositCashAddValue">
							</div>
						</div>
					</div>
				</div>
