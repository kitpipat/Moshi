...
?>
	...
	$(document).ready(function() {
		...
		if (tDocNo != '' && (tStaApv == 2 || tStaPrcStk == 2)) { // 2 = Processing
			...
			FSxCMNRabbitMQMessageTestForTwx(poDocConfig, poMqConfig, poUpdateStaDelQnameParams, poCallback);
		}
	...
	var obtTFWBrowseWahStart;
	var oTFWBrowseShpEnd;
	var oTFWBrowsePosEnd;
	var obtTFWBrowseWahEnd;
	...
	//สาขาจบ
	$('#obtTFWBrowseBchEnd').click(function() {
		...
		tOldBchCkChange = $("#oetBchCodeEnd").val();
		//Lang Edit In Browse
		nLangEdits = <?php echo $this->session->userdata("tLangEdit") ?>;
		//Set Option Browse -----------
		//Option Branch
		oPmhBrowseBch = {
			...
			Where: {
				Condition: ["AND TCNMBranch.FTBchCode NOT IN ('" + $("#oetBchCodeStart").val() + "')"]
			},
			GrideView: {
			...
			CallBack: {
				ReturnType: 'S',
				Value: ["oetBchCodeEnd", "TCNMBranch.FTBchCode"],
				Text: ["oetBchNameEnd", "TCNMBranch_L.FTBchName"],
			},
			NextFunc: {
	...
	$('#obtTFWBrowseWahStart').click(function() {
		...
		//Option WareHouse From 
		JCNxBrowseData('obtTFWBrowseWahStart');
	});
	...
	$('#obtTFWBrowseWahEnd').click(function() {
		...
		//Option WareHouse From 
		JCNxBrowseData('obtTFWBrowseWahEnd');
	});
	...
	$('#oliBtnEditShipAdd').click(function() {
		...
		oTFWBrowseShipAdd = {
			...
			Where: {
				Condition: [
					function() {
						var tFilter = "AND FTAddGrpType = 1 AND FTAddRefCode = '" + $("#oetBchCodeEnd").val() + "' AND TCNMAddress_L.FNLngID = " + nLangEdits;
						return tFilter;
					}
				]
			},
