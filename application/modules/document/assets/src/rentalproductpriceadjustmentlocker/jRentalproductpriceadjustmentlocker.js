...
function FSvPDTAddPdtIntoTableDT(pjPdtData) {
  ...
  if (typeof nStaSession !== "undefined" && nStaSession == 1) {
    ...
    JCNxOpenLoading();
    var ptXthDocNoSend = "";
    if ($("#ohdTFWRoute").val() == "ADJPLEventEdit") {
      ptXthDocNoSend = $("#oetXthDocNo").val();
    }

    $.ajax({
      type: "POST",
      url: "ADJPLAddPdtIntoTableDT",
      data: {
...
function JSvCallPageTFWAdd() {
  ...
  $.ajax({
    ...
    success: function (tResult) {
      ...
      $("#ocmPriceType,#oetXthDocNo,#oetXthDocDate,#oetXthDocTime,#oetXthDocDateStart,#oetXthDocDateEnd,#oetXthDocTimeStart,#oetXthDocTimeEnd").bind("blur change",function () {
      ...
      if($("#ohdUserType").val()!="SHP"){
        $("#obtTWXVDControlFormClear").click(function(){
          JCNxOpenLoading();
          $.ajax({
            type: "POST",
            url: "ADJPLClearDocTemForChngCdt",
            data: {
            ...
            success: function (tResult) {
              ...
              JCNxCloseLoading();
              FSvCMNSetMsgWarningDialog("<p>ข้อมูลเลย์เอาต์เดิมจะถูกเคลียร์ทิ้งเมื่อท่านล้างข้อมูล</p>");
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      //JCNxResponseError(jqXHR, textStatus, errorThrown);
      FSvCMNSetMsgWarningDialog("<p>เครื่อข่ายไม่ตอบสนองโปรดลองอีกครั้ง</p>");
    }
  });
}
...
function JSxValidateFormAddTFW() {
  ...
  $.validator.addMethod('datecompare', function (value, element) {
    if ($('#oetXthDocDateStart').val()!="" &&
        $('#oetXthDocDateEnd').val()!="") {
          var tXthDocDateStart = new Date($('#oetXthDocDateStart').val());
          var tXthDocDateEnd = new Date($('#oetXthDocDateEnd').val());
          if(tXthDocDateStart<=tXthDocDateEnd){
            return true;
          }else{
            return false;
          }
  ...
  $.validator.addMethod('timecompare', function (value, element) {
    if ($('#oetXthDocTimeStart').val()!="" &&
        $('#oetXthDocTimeEnd').val()!="") {
          var dDate = new Date();
          var dDateFormat = dDate.getFullYear()+'-'+(dDate.getMonth()+1)+'-'+dDate.getDate();
          var tXthDocTimeStart = new Date(dDateFormat+" "+$('#oetXthDocTimeStart').val());
          var tXthDocTimeEnd = new Date(dDateFormat+" "+$('#oetXthDocTimeEnd').val());
          if(tXthDocTimeStart<tXthDocTimeEnd){
            return true;
          }else{
            return false;
          }
  ...
  $('#ofmAddTFW').validate({
    ...
    rules: {
      oetXthDocNo: {
        "required": {
          depends: function (oElement) {
            if ($("#ohdTFWRoute").val() == "ADJPLEventAdd") {
              if ($('#ocbTFWAutoGenCode').is(':checked')) {
                return false;
              } else {
                return true;
    ...
    messages: {
      ...
      oetMchName: {
        "required": $('#oetMchName').attr('data-validate-required')
      },
      oetShpNameStart: {
        "required": $('#oetShpNameStart').attr('data-validate-required')
      },
      oetXthDocDateStart:{
        "required": $('#oetXthDocDateStart').attr('data-validate-required'),
        "datecompare": "วันที่มีผลต้องน้อยกว่าหรือเท่ากับวันที่สิ้นสุด"
      },
      oetXthDocDateEnd:{
        "required": $('#oetXthDocDateEnd').attr('data-validate-required')
      },
      oetXthDocTimeStart:{
        "required": $('#oetXthDocTimeStart').attr('data-validate-required'),
        "timecompare": "เวลาที่มีผลต้องน้อยกว่าเวลาที่สิ้นสุด"
      },
      oetXthDocTimeEnd:{
        "required": $('#oetXthDocTimeEnd').attr('data-validate-required')
      }
    },
    errorElement: "em",
    errorPlacement: function (error, element) {
...
function JSxValidateTFWCodeDublicate() {
  ...
  $.ajax({
    ...
    success: function (tResult) {
      ...
      // $.ajax({
      //   type: "POST",
      //   url: "ADJPLCheckDateTime",
      //   data: {
      //     tDocno : $("#oetXthDocNo").val(),
      //     tPriceType : $("#ocmPriceType").val(),
      //     tXthDocDateStart : $("#oetXthDocDateStart").val(),
      //     tXthDocDateEnd : $("#oetXthDocDateEnd").val(),
      //     tXthDocTimeStart : $("#oetXthDocTimeStart").val(),
      //     tXthDocTimeEnd : $("#oetXthDocTimeEnd").val()
      //   },
      //   cache: false,
      //   timeout: 0,
      //   success: function (tResult) {
          //alert(JSON.stringify(tResult));
          // var bResult = JSON.parse(tResult);

          // if(bResult){
          //   $("#ohdCheckDataTimeHDDuplicate").val(0);
          // }else{
          //   $("#ohdCheckDataTimeHDDuplicate").val(1);
          // }
          ...
          // $.validator.addMethod('dublicateDateTime', function (value, element) {
          //   if($("#ohdCheckDataTimeHDDuplicate").val()==1){
          //     return false;
          //   }else{
          //     return true;
          //   }
          ...
          $('#ofmAddTFW').validate({
            ...
            messages: {
              ...
              // oetXthDocDateStart:{
              //   "dublicateDateTime":"วันที่และเวลาทับซ้อนกับข้อมูลในฐานข้อมูลโปรดเปลี่ยนวันที่และเวลา"
              // }
            },
            errorElement: "em",
            errorPlacement: function (error, element) {
      ...
      //   },
      //   error: function (jqXHR, textStatus, errorThrown) {
      //     FSvCMNSetMsgWarningDialog("<p>ตรวจพบปัญหาด้านเครื่อข่ายโปรดลองอีกครั้ง</p>");
      //   }
      // });
...
function JSxSubmitEventByButton() {
  ...
  $.ajax({
    type: "POST",
    url: "ADJPLCheckPdtTmpForTransfer",
    data: { 
      tBchCode:$("#ohdBchCode").val(),
      tFTBchCodeForADJPL:$("#oetBchCode").val(),
      tMerCodeForADJPL:$("#oetMchCode").val(),
      tShpCodeForADJPL:$("#oetShpCodeStart").val(),
    ...
    success: function (tResult) {
      ...
      if (bReturn) {
        ...
        FSvCMNSetMsgWarningDialog("<p>โปรดระบุราคาของขนาดเพื่อทำการปรับราคา</p>");
      }
      ...
      if(!($("#otbDOCPdtTable tbody tr td[colspan='100%']").length==1)){ // ถ้าพบว่าข้อมูล 1 แถวนั้นคือ ข้อมูล Layout สินค้า (ในโค๊ดกล่าวตรงข้ามคือ ถ้าไม่ใช่ แถวที่แสดงข้อมูลว่าไม่พบข้อมูล)
        ...
        $.ajax({
          ...
          success: function (tResult) {
            ...
            $("#odvPaginationBtn").html(tHtml);
            JCNxCloseLoading();
            FSvCMNSetMsgWarningDialog("<p>ข้อมูลเลย์เอาต์เดิมจะถูกเคลียร์ทิ้งเมื่อท่านเลือกสาขาใหม่</p>");
      ...
      if(!($("#otbDOCPdtTable tbody tr td[colspan='100%']").length==1)){ // ถ้าพบว่าข้อมูล 1 แถวนั้นคือ ข้อมูล Layout สินค้า (ในโค๊ดกล่าวตรงข้ามคือ ถ้าไม่ใช่ แถวที่แสดงข้อมูลว่าไม่พบข้อมูล)
        ...
        $.ajax({
          ...
          success: function (tResult) {
            ...
            $("#odvPaginationBtn").html(tHtml);
            JCNxCloseLoading();
            FSvCMNSetMsgWarningDialog("<p>ข้อมูลเลย์เอาต์เดิมจะถูกเคลียร์ทิ้งเมื่อท่านเลือกร้านค้าใหม่</p>");
...
function JSvCallPageTFWList() {
  $.ajax({
    ...
    success: function (tResult) {
      ...
      $("#oetMenSearch").removeAttr("disabled");
      $("#odvBtnTFWInfo .xCNBTNPrimeryPlus").removeAttr("disabled");
    },
    error: function (jqXHR, textStatus, errorThrown) {
      JCNxResponseError(jqXHR, textStatus, errorThrown);
    }
  ...
  $.ajax({
    ...
    success: function (tResult) {
      ...
      $('#obtTFWPrint').bind("click",function(){
        ...
        window.open($("#ohdBaseUrl").val()+"formreport/RTPdtBillAdjustPriceLK?infor="+JCNtEnCodeUrlParameter(aInfor), '_blank');
      });
...
function JSvTFWLoadPdtDataTableHtml(pnPage) {
  ...
  if (typeof nStaSession !== "undefined" && nStaSession == 1) {
    ...
    let oInfor = {
      tBchCode : $("#oetBchCode").val(),
      tMerCode : $("#oetMchCode").val(),
      tShpCode : $("#oetShpCodeStart").val(),
...
function JSxLoadPdtDtFromTem(pnPage,oInfor){
  $.ajax({
    type: "POST",
    url: "ADJPLPdtAdvanceTableLoadData",
    data: oInfor,
    cache: false,
    Timeout: 0,
    success: function (tResult) {
...
function JSxSaveRateCodeToTmp(){
  var bCheckNoData = false;
  if($("#otbDOCPdtTableRateModalOver tbody tr").length==1){
    if($("#otbDOCPdtTableRateModalOver tbody tr td[colspan='100%']").length!=0){
      var bCheckNoData = true;
    }
  }
  if(bCheckNoData==false){
    JCNxOpenLoading();
    let tOnlickFunction = false;
    let bCheckNoSelect = false;
    if($("#otbDOCPdtTableRateModalOver tbody tr.active").length!=0){
      tOnlickFunction = $("#otbDOCPdtTableRateModalOver tbody tr.active").attr("onclick");
    }else{
      if($("#otbDOCPdtTableRateModalOver tbody tr").length!=0){
        tOnlickFunction = $($("#otbDOCPdtTableRateModalOver tbody tr").eq(0)).attr("onclick");
        bCheckNoSelect = true;
      }
    }
    if(tOnlickFunction){
      let tParameter = tOnlickFunction.substring(tOnlickFunction.indexOf("{")+1,tOnlickFunction.indexOf("}")).replace("'","");
      while(true){
        if(tParameter.indexOf("'")!=-1){
          tParameter = tParameter.replace("'","");
        }else{
          break;
        }
      }
      let aParameter = tParameter.split(",");
      let oInforFild =  [
                          aParameter[0].split(":")[0], 
                          aParameter[1].split(":")[0], 
                          aParameter[2].split(":")[0], 
                          aParameter[3].split(":")[0], 
                          aParameter[4].split(":")[0], 
                          aParameter[5].split(":")[0], 
                          aParameter[6].split(":")[0], 
                          aParameter[7].split(":")[0], 
                          aParameter[8].split(":")[0],
                          aParameter[9].split(":")[0]
                        ]; 
      let oInforData =  [
                          aParameter[0].split(":")[1], 
                          aParameter[1].split(":")[1], 
                          aParameter[2].split(":")[1], 
                          aParameter[3].split(":")[1], 
                          aParameter[4].split(":")[1], 
                          aParameter[5].split(":")[1], 
                          aParameter[6].split(":")[1], 
                          aParameter[7].split(":")[1], 
                          aParameter[8].split(":")[1],
                          aParameter[9].split(":")[1]
                        ]; 
      if(bCheckNoSelect){
        for(let nI=0;nI<oInforFild.length;nI++){
          if(oInforFild[nI]=="FTRthCodeForADJPL" ||
            oInforFild[nI]=="FTRthCodeForADJPLSelect"){
              oInforData[nI] = "";
          }
        }
      }
      $.ajax({
        type: "POST",
        url: "ADJPLPdtSaveRateDTInTmp",
        data : {
                  [oInforFild[0]] : oInforData[0],
                  [oInforFild[1]] : oInforData[1],
                  [oInforFild[2]] : oInforData[2],
                  [oInforFild[3]] : oInforData[3],
                  [oInforFild[4]] : oInforData[4],
                  [oInforFild[5]] : oInforData[5],
                  [oInforFild[6]] : oInforData[6],
                  [oInforFild[7]] : oInforData[7],
                  [oInforFild[8]] : oInforData[8],
                  [oInforFild[9]] : oInforData[9]
        },
        cache: false,
        Timeout: 0,
        success: function (tResult) {
  ...
  $("#odvADJPLRateToRental").modal("hide");
}

function JSxSelectRateHD(poElement,poInfor){
  JCNxOpenLoading();
  if(!$(poElement).hasClass("active")){
    $.ajax({
      type: "POST",
      url: "ADJPLPdtGetRateDTInfor",
      data : {"tRateCode":poInfor.FTRthCodeForADJPLSelect},
      cache: false,
      Timeout: 0,
      success: function (tResult) {
        $("#otbDOCPdtTableRateModalOver tbody tr").removeClass("active");
        $(poElement).addClass("active");
        var tData = $.parseJSON(tResult);
        
        if(tData){
          let tHtmlDT = "";
          for(let nJ=0;nJ<tData.length;nJ++){
            tHtmlDT += "<tr>";
            tHtmlDT += "  <td>";
            tHtmlDT += "    <label>";
            tHtmlDT +=        tData[nJ]["FNRtdSeqNo"];
            tHtmlDT += "    </label>";
            tHtmlDT += "  </td>";
            tHtmlDT += "  <td>";
            tHtmlDT += "    <label>";
            tHtmlDT +=        tData[nJ]["FNRtdMinQty"];
            tHtmlDT += "    </label>";
            tHtmlDT += "  </td>";
            if(tData[nJ]["FTRtdTmeType"]==1){
              tHtmlDT += "  <td>";
              tHtmlDT += "    <label>";
              tHtmlDT +=        "นาที";
              tHtmlDT += "    </label>";
              tHtmlDT += "  </td>";
            }else if(tData[nJ]["FTRtdTmeType"]==2){
              tHtmlDT += "  <td>";
              tHtmlDT += "    <label>";
              tHtmlDT +=        "ชั่วโมง";
              tHtmlDT += "    </label>";
              tHtmlDT += "  </td>";
            }else if(tData[nJ]["FTRtdTmeType"]==3){
              tHtmlDT += "  <td>";
              tHtmlDT += "    <label>";
              tHtmlDT +=        "วัน";
              tHtmlDT += "    </label>";
              tHtmlDT += "  </td>";
            }else if(tData[nJ]["FTRtdTmeType"]==4){
              tHtmlDT += "  <td>";
              tHtmlDT += "    <label>";
              tHtmlDT +=        "เดือน";
              tHtmlDT += "    </label>";
              tHtmlDT += "  </td>";
            }else if(tData[nJ]["FTRtdTmeType"]==5){
              tHtmlDT += "  <td>";
              tHtmlDT += "    <label>";
              tHtmlDT +=        "ปี";
              tHtmlDT += "    </label>";
            ...
            tHtmlDT += "  <td>";
            tHtmlDT += "    <label>";
            tHtmlDT +=        tData[nJ]["FCRtdPrice"];
            tHtmlDT += "    </label>";
            tHtmlDT += "  </td>";
            tHtmlDT += "</tr>";
          }
          $("#otbDOCPdtTableRateModalUnder tbody").html(tHtmlDT);
        }
        JCNxCloseLoading();
      },
      error: function (jqXHR, textStatus, errorThrown) {
    ...
    $("#otbDOCPdtTableRateModalOver tbody tr").removeClass("active");
    $("#otbDOCPdtTableRateModalUnder tbody").html("<tr><td colspan=\"100%\" class=\"text-center\"><span>ไม่พบข้อมูล</span></td></tr>");
    JCNxCloseLoading();
  }
...
function JSxSetToRateRentalModalSelect(poElement,poInfor){
  JCNxOpenLoading();
  $.ajax({
    type: "POST",
    url: "ADJPLPdtGetRateInfor",
    cache: false,
    Timeout: 0,
    success: function (tResult) {
      var tData = $.parseJSON(tResult);
      var tHtmlHD = "";
      if(tData){
        for(let nI=0;nI<tData.length;nI++){
          tHtmlHD += "<tr class=\"xWRateHDSelected";
          tHtmlHD += "\" dataRateCode=\""+tData[nI]["aInforHD"]["FTRthCode"]+"\" onclick=\"JSxSelectRateHD(this,{";
          tHtmlHD += "'FTBchCode':'"+poInfor.FTBchCode+"',";
          tHtmlHD += "'FTBchCodeForADJPL':'"+poInfor.FTBchCodeForADJPL+"',";
          tHtmlHD += "'FTMerCodeForADJPL':'"+poInfor.FTMerCodeForADJPL+"',";
          tHtmlHD += "'FTShpCodeForADJPL':'"+poInfor.FTShpCodeForADJPL+"',";
          tHtmlHD += "'FTXthDocNo':'"+poInfor.FTXthDocNo+"',";
          tHtmlHD += "'FTXthDocKey':'"+poInfor.FTXthDocKey+"',";
          tHtmlHD += "'FTSessionID':'"+poInfor.FTSessionID+"',";
          tHtmlHD += "'FTPzeCodeForADJPL':'"+poInfor.FTPzeCodeForADJPL+"',";
          tHtmlHD += "'FTRthCodeForADJPL':'"+poInfor.FTRthCodeForADJPL+"',";
          tHtmlHD += "'FTRthCodeForADJPLSelect':'"+tData[nI]["aInforHD"]["FTRthCode"]+"'";
          tHtmlHD += "});\">";
          tHtmlHD += "  <td>";
          tHtmlHD += "    <label>";
          tHtmlHD +=        tData[nI]["aInforHD"]["FTRthCode"];
          tHtmlHD += "    </label>";
          tHtmlHD += "  </td>";
          tHtmlHD += "  <td>";
          tHtmlHD += "    <label>";
          tHtmlHD +=        tData[nI]["aInforHD"]["FTRthName"];
          tHtmlHD += "    </label>";
          tHtmlHD += "  </td>";
          tHtmlHD += "  <td>";
          if(tData[nI]["aInforHD"]["FTRthCalType"]==1){
            tHtmlHD += "    <label>";
            tHtmlHD +=        "ปัดขึ้น";
            tHtmlHD += "    </label>";
          }else if(tData[nI]["aInforHD"]["FTRthCalType"]==2){
            tHtmlHD += "    <label>";
            tHtmlHD +=        "ปัดลง";
            tHtmlHD += "    </label>";
          }else if(tData[nI]["aInforHD"]["FTRthCalType"]==3){
            tHtmlHD += "    <label>";
            tHtmlHD +=        "เฉลี่ย";
            tHtmlHD += "    </label>";
          }
        ...
        //odvADJPLRateToRental  modal-body modal-footer
        $("#otbDOCPdtTableRateModalOver tbody").html(tHtmlHD);
      }else{
        $("#otbDOCPdtTableRateModalOver tbody").html("<tr><td colspan=\"100%\" class=\"text-center\"><span>ไม่พบข้อมูล</span></td></tr>");
      }
      
      if(poInfor.FTRthCodeForADJPL!=""){
        let oElementGrp = $("#otbDOCPdtTableRateModalOver tbody tr");
        let nIndex = 0;
        for(let nI=0;nI<oElementGrp.length;nI++){
          if($(oElementGrp.eq(nI)).attr("dataRateCode")==poInfor.FTRthCodeForADJPL){
            nIndex = nI;
            break;
          }
        }
        $($("#otbDOCPdtTableRateModalOver tbody tr").eq(nIndex)).click();
      }else{
        $("#otbDOCPdtTableRateModalUnder tbody").html("<tr><td colspan=\"100%\" class=\"text-center\"><span>ไม่พบข้อมูล</span></td></tr>");
      }
      JCNxCloseLoading();
      $("#odvADJPLRateToRental").modal("show");
    },
    error: function (jqXHR, textStatus, errorThrown) {
      JCNxResponseError(jqXHR, textStatus, errorThrown);
    }
                              ...
                              if(bCheckCompare){
                                  if(nValue>=nDataMin&&nValue<=nDataMax){
                                    ...
                                    if(nValue!=0){
                                      ...
                                      $($(".xWEditInlineElement").eq(nDataSeq)).val(nDataMax);
                                      $($(".xWEditInlineElement").eq(nDataSeq)).attr("style","background: #F9F9F9;border-color: #a94442;-webkit-box-shadow: inset 0 1px 1px rgba(0, 0, 0, .075);box-shadow: inset 0 1px 1px rgba(0, 0, 0, .075);"+$($(".xWEditInlineElement").eq(nDataSeq)).attr("style"));
                                      $($(this).parent()).append("<em class=\"error help-block\">กรุณากรอกตัวเลขที่ไม่เป็น 0</em>");
...
function JSxOpenColumnFormSet() {
  $.ajax({
    type: "POST",
    url: "ADJPLAdvanceTableShowColList",
    data: {},
    cache: false,
    Timeout: 0,
    success: function (tResult) {
...
function JSxSaveColumnShow() {
  ...
  $.ajax({
    type: "POST",
    url: "ADJPLAdvanceTableShowColSave",
    data: {
