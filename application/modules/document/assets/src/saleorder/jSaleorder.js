...
$("document").ready(function(){
    ...
    if(typeof(nSOStaSOBrowseType) != 'undefined' && nSOStaSOBrowseType == 0){
        ...
        $('#obtSOCopyDoc').click(function(){
            // JCNxOpenLoading();
            var tDocno = $("#oetSODocNo").val();
            var ChkDis = $('#olbDisChgHD').text();
            if(ChkDis == ''){
                JSxSOCopyDocument('1');
            }else{
                $('#odvSOModalCopyDoc').modal({backdrop:'static',keyboard:false});
                $("#odvSOModalCopyDoc").modal("show");
            }
        });

        // Event Click Cancel Document
        $('#obtSOCancelDoc').unbind().click(function(){
            var tStaref = $("#ohdSOStaRef").val();
            if(tStaref != '0'){
                FSvCMNSetMsgWarningDialog('ไม่สามารถยกเลิกเอกสารที่มีการอ้างอิงแล้วได้');
                return false;
            }
            var nStaSession = JCNxFuncChkSessionExpired();
            if(typeof(nStaSession) !== "undefined" && nStaSession == 1) {
...
function JSxSOCopyDocument(pbIsConfirm){
    $("#odvSOModalCopyDoc").modal("hide");
    ...
    $.ajax({
        ...
        success: function (oResult){
            ...
            JSvSOCallPageEditDoc(aReturnData['SoDocNew'],2);
            JCNxCloseLoading();
        },
...
// Return: Status Add/Update Document
// ReturnType: object
function JSvSOCallPageEditDoc(ptSODocNo,ptProrate = 1){
    var nStaSession = JCNxFuncChkSessionExpired();
    if(typeof nStaSession !== "undefined" && nStaSession == 1) {
        $('#obtSOCopyDoc').show();
        JCNxOpenLoading();
...
function JCNxSOControlObjAndBtn(){
    ...
    if(nSOStaDoc != 1){
        ...
        $("#obtSOCopyDoc").hide(); // attr("disabled",true);
