...
class cSaleOrder extends MX_Controller {
    ...
    public function FSoCSOEditPage() {
        ...
        try {
            ...
            $tChkProrate = $this->input->post('ptProrate');
...
//-----------------------------------------------------------------------------
    ...
    // Return : Object View Page Add
    // Return Type : object
    public function FSoCSOCopyPage() {
        // die();

        try {
            $tSODocNo = $this->input->post('tDocno');
            $tGetDis  = $this->input->post('tGetDis');
            ...
            $aSOGenCode = FCNaHGenCodeV5($aTableAddUpdate['tTableHD'], $aTableAddUpdate['tTableStaGen']);
            if ($aSOGenCode['rtCode'] == '1') {
                $tSODocNoNew = $aSOGenCode['rtXshDocNo'];
            }

            $aDataWhere = array(
                'tSODocNo'      => $tSODocNo,
                'tSODocNoNew'   => $tSODocNoNew,
                'tGetDis'       => $tGetDis,
            );
           
            // Copy Data Document
            $aDataDocHD = $this->mSaleOrder->FSaMSOCopyData($aDataWhere);
            FCNaHCalculateProrate('TARTSoHD', $tSODocNoNew);
            $aReturnData = array(
                'SoDocNew'  => $tSODocNoNew,
                'nStaEvent' => '1',
                'tStaMessg' => 'Success'
            );
