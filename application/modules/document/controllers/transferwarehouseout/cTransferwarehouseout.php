...
class cTransferwarehouseout extends MX_Controller{
    ...
    public function FSoCTWOAddPdtIntoDocDTTemp() {
        try {
            ...
            $tTWOBchCode         = ($tTWOUserLevel == 'HQ') ? FCNtGetBchInComp() : $this->session->userdata("tSesUsrBchCode");
    ...
    public function FSoCTWOAddEventDoc(){
        try {
            ...
            if ($tTWOAutoGenCode == '1') {
                $aTWOGenCode = FCNaHGenCodeV5($aTableAddUpdate['tTableHD'], $aTableAddUpdate['tTableStaGen']);

              
                if ($aTWOGenCode['rtCode'] == '1') {
                    $aDataWhere['FTXthDocNo'] = $aTWOGenCode['rtXthDocNo'];
                }
            } else {
