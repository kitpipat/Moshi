...
class mSalePriceAdj extends CI_Model {
    ...
    public function FSaMSPAList($paData){
        ...
        try{
            ...
            $oAdvanceSearch     = $paData['oAdvanceSearchData'];
            $tSQL       = " SELECT c.* FROM(
            ...
            if($this->session->userdata("tSesUsrLevel") == "HQ"){
                ...
                $tSQL .= " AND PPH.FTBchCode = '".$aDataUserInfo['FTBchCode']."'";
                if($this->session->userdata("tSesUsrLevel")=="SHP"){
                    // $tSQL .= " AND PPH.FTXthShopFrm = '".$aDataUserInfo['FTShpCode']."'";
                }
            }
            ...
            if($this->session->userdata("tSesUsrLevel") != "HQ"){
            $tSQL .=" OR ( ";
            $tSQL .= " PPH.FTXphStaApv=1 AND BCHPPL.FTBchCode='".$aDataUserInfo['FTBchCode']."'  ";
            $tSQL .=" ) ";
            }
            ...
            $aDataGetRow = array(
                'tSearchList'               => $tSearchList,
                'tSearchDocDateFrom'        => $tSearchDocDateFrom,
                'tSearchDocDateTo'          => $tSearchDocDateTo,
                'tSearchBchCodeFrom'        => $tSearchBchCodeFrom,
                'tSearchBchCodeTo'          => $tSearchBchCodeTo,
                'tSearchStaDoc'             => $tSearchStaDoc,
                'tSearchStaApprove'         => $tSearchStaApprove,
                'tSearchStaPrcStk'          => $tSearchStaPrcStk,
            );
            ...
            if($oQuery->num_rows() > 0){
                ...
                $oFoundRow = $this->FSoMSPAGetListPageAll($aDataGetRow);
    ...
    //Return Type : Object
    public function FSoMSPAGetListPageAll($paData){
        try{
            $aDataUserInfo          = $this->session->userdata("tSesUsrInfo");
            @$tSearchList           = $paData['tSearchList'];
            $tSearchDocDateFrom     = $paData['tSearchDocDateFrom'];
            $tSearchDocDateTo       = $paData['tSearchDocDateTo'];
            $tSearchBchCodeFrom     = $paData['tSearchBchCodeFrom'];
            $tSearchBchCodeTo       = $paData['tSearchBchCodeTo'];
            $tSearchStaDoc          = $paData['tSearchStaDoc'];
            $tSearchStaApprove      = $paData['tSearchStaApprove'];
            $tSearchStaPrcStk       = $paData['tSearchStaPrcStk'];

            $tSQL = "SELECT COUNT (DISTINCT PPH.FTXphDocNo) AS counts
    ...
    public function FSoMSPAGetPdtPriPageAll($ptSearchList,$ptXphDocNo,$FTSessionID,$nLngID,$FTXthDocKey){
        try{
            $tSQL = "SELECT 
                        COUNT (DTP.FTXthDocNo) AS counts, 
                        sum(DTP.FCXtdPriceRet) AS FCXtdPriceRet, 
                        sum(DTP.FCXtdPriceWhs) AS FCXtdPriceWhs, 
                        sum(DTP.FCXtdPriceNet) AS FCXtdPriceNet
                     FROM TCNTDocDTTmp DTP WITH(NOLOCK)
                     LEFT JOIN TCNMPdt_L PDT_L      ON DTP.FTPdtCode = PDT_L.FTPdtCode AND PDT_L.FNLngID = $nLngID
                     LEFT JOIN TCNMPdtUnit_L PUN_L  ON DTP.FTPunCode = PUN_L.FTPunCode AND PUN_L.FNLngID = $nLngID
                     LEFT JOIN TCNMShop_L SHP_L     ON DTP.FTXtdShpTo = SHP_L.FTShpCode AND DTP.FTBchCode = SHP_L.FTBchCode AND SHP_L.FNLngID = $nLngID
                     LEFT JOIN TCNMBranch_L BCH_L   ON DTP.FTXtdBchTo = BCH_L.FTBchCode AND BCH_L.FNLngID = $nLngID
                     WHERE DTP.FTXthDocNo = '$ptXphDocNo' AND DTP.FTSessionID = '$FTSessionID' AND DTP.FTXthDocKey = '$FTXthDocKey'";
    ...
    public function FSaMSPAGetDataByID($paData){
        try{
            ...
            $tSQL       = "SELECT
                                BCH_L.FTBchName     AS FTBchName,
                                ZNE_L.FTZneName     AS FTZneName,
                                AGG_L.FTAggName     AS FTAggName,
                                PPL_L.FTPplName     AS FTPplName,
                                CUSR_L.FTUsrName    AS FTCreateBy,
                                CUSR_L.FTUsrName    AS FTUsrName,
                                AUSR_L.FTUsrName    AS FTXphUsrApv,
                                MER_L.FTMerName     AS FTMerName,
                                PPH.*
                            FROM TCNTPdtAdjPriHD PPH
                            LEFT JOIN TCNMBranch_L      BCH_L   ON PPH.FTXphBchTo   = BCH_L.FTBchCode   AND BCH_L.FNLngID   = $nLngID
    ...
    public function FSnMSPACheckDuplicate($ptXphDocNo){
        $tSQL = "SELECT COUNT(PPH.FTXphDocNo) AS counts
                 FROM TCNTPdtAdjPriHD PPH 
    ...
    public function FSaMSPAAddUpdateMaster($paData){
        try{
            ...
            // ///--------------Save Log-------------///
            //     $tQueryLog = $this->db->last_query();
            //     $dDateTime = date('Y-m-d H:i:s');
            //     $content = "\r\ $dDateTime \r\ $tQueryLog \r\n";
            //     $fp = fopen(FCPATH . "/log/Log_SalePriceAdj_".date('Y-m-d').".txt","a+");
            //     fwrite($fp,$content);
            //     fclose($fp);
            // ///--------------Save Log-------------///
    ...
    public function FSaMSPAAddPdtDocTmp($paData){
        try{
            ...
            $this->db->insert('TCNTDocDTTmp', array(
                ...
                'FCXtdPriceRet'   => $paData['FCXtdPriceRet'],
    ...
    public function FSaMSPAUpdatePriceTemp($paData){
        try{
            ...
            switch($paData['tPrice']){
                case "FCXtdPriceRet":
                    $aDataUpdate = array(
                        'FCXtdPriceRet' => $paData['tValue']
                    );
                    break;
                case "FCXtdPriceNet":
                    $aDataUpdate = array(
                        'FCXtdPriceNet' => $paData['tValue']
                    );
                    break;
                case "FCXtdPriceWhs":
                    $aDataUpdate = array(
                        'FCXtdPriceWhs' => $paData['tValue']
                    );
                    break;
                case "All":
                    $aDataUpdate = array(
            ...
            $this->db->update('TCNTDocDTTmp',$aDataUpdate);
    ...
    public function FSoMSPADTtoTmp($paData){
        try{
            ...
            $tSQL = "INSERT INTO TCNTDocDTTmp (FTBchCode,FTXthDocNo,FNXtdSeqNo,FTPdtCode,FTPunCode,FCXtdPriceRet,FCXtdPriceWhs,FCXtdPriceNet,FTXtdShpTo,FTXtdBchTo,FTSessionID,FTXthDocKey,FDLastUpdOn,FDCreateOn,FTLastUpdBy,FTCreateBy)
                        SELECT FTBchCode,FTXphDocNo,FNXpdSeq,FTPdtCode,FTPunCode,FCXpdPriceRet,FCXpdPriceWhs,FCXpdPriceNet,FTXpdShpTo,FTXpdBchTo,'$FTSessionID','$FTXthDocKey',FDLastUpdOn,FDCreateOn,FTLastUpdBy,FTCreateBy
    ...
    public function FSoMSPATmptoDT($paData){
        try{
            ...
            (FTBchCode,FTXphDocNo,FNXpdSeq,FTPdtCode,FTPunCode,FCXpdPriceRet,FCXpdPriceWhs,FCXpdPriceNet,FTXpdShpTo,FTXpdBchTo,FDLastUpdOn,FDCreateOn,FTLastUpdBy,FTCreateBy)
            SELECT 
                FTBchCode,
                FTXthDocNo,
                ...
                '$FTCreateBy'
            FROM TCNTDocDTTmp
            WHERE FTXthDocNo = '$FTXphDocNo' AND FTSessionID = '$FTSessionID' AND FTXthDocKey = '$FTXthDocKey'";
    ...
    public function FSaMSPACheckDataSeq($paData){
        ...
        try{
            ...
            $tSQL = "SELECT MAX(FNXtdSeqNo) AS nSeq
                     FROM TCNTDocDTTmp
                     WHERE FTXthDocNo = '$FTXthDocNo' AND FTSessionID = '$FTSessionID' AND FTXthDocKey = '$FTXthDocKey'";
            $oQuery = $this->db->query($tSQL);
    ...
    public function FSaMSPADataPrice4Pdt($paData){
        ...
        try{
            ...
            $tSQL = "SELECT TOP 1 PDT_L.FTPdtName,PRI.FTPdtCode,PUN_L.FTPunName,PRI.FCPgdPriceRet,PRI.FCPgdPriceWhs,PRI.FCPgdPriceNet
                     FROM ".$tTable." PRI
