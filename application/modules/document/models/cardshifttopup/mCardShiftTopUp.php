...
class mCardShiftTopUp extends CI_Model {
    ...
    public function FSaMCardShiftTopUpSearchByID($ptAPIReq, $ptMethodReq, $paData){
        $tCardShiftTopUpDocNo = $paData['FTCthDocNo'];
        $nLngID = $paData['FNLngID'];
        $tSQL = "
            SELECT
                CRDHD.FTCthDocNo AS rtCardShiftTopUpDocNo,
                CRDHD.FTBchCode AS rtCardShiftTopUpBchCode,
                CRDHD.FTCthDocType AS rtCardShiftTopUpDocType,
                CRDHD.FDCthDocDate AS rtCardShiftTopUpDocDate,
                CRDHD.FTUsrCode AS rtCardShiftTopUpUsrCode,
                CRDHD.FTCthApvCode AS rtCardShiftTopUpApvCode,
                CRDHD.FDCthApvDate AS rtCardShiftTopUpApvDate,
                CRDHD.FCCthTotalQty AS rtCardShiftTopUpCardQty,
                CRDHD.FCCthAmtTP AS rtCardShiftTopUpAmtTP,
                CRDHD.FCCthTotalTP AS rtCardShiftTopUpTotalTP,
                CRDHD.FTCthStaPrcDoc AS rtCardShiftTopUpStaPrcDoc,
                CRDHD.FTCthStaDoc AS rtCardShiftTopUpStaDoc,
                CRDHD.FTCthStaDelMQ AS rtCardShiftTopUpStaDelMQ
            FROM [TFNTCrdTopUpHD] CRDHD WITH(NOLOCK)
            WHERE 1=1";
            $tSQL .= " AND CRDHD.FTCthDocNo = '$tCardShiftTopUpDocNo'";
            $tSQL .= " AND CRDHD.FTCthDocType = '1' /* 1: Top Up */
        ";
    ...
    public function FSaMCardShiftTopUpList($ptAPIReq, $ptMethodReq, $paData){
        ...
        $nLngID = $paData['FNLngID'];
        $tSQL = "SELECT c.* FROM(
                    SELECT ROW_NUMBER() OVER(ORDER BY rtCardShiftTopUpDocDate DESC) AS rtRowID,*
                    FROM
                        (SELECT DISTINCT
                            CRDHD.FTBchCode AS rtCardShiftTopUpBchCode,
                            CRDHD.FTCthDocNo AS rtCardShiftTopUpDocNo,
                            CRDHD.FDCthDocDate AS rtCardShiftTopUpDocDate,
                            CRDHD.FCCthTotalQty AS rtCardShiftTopUpCthCardQty,
                            CRDHD.FTCthStaDoc AS rtCardShiftTopUpCthStaDoc,
                            CRDHD.FTCthStaPrcDoc AS rtCardShiftTopUpCthStaPrcDoc
                        FROM [TFNTCrdTopUpHD] CRDHD
                        WHERE 1=1";
        
                        // BchCode is empty = HQ(use all)
                        ($tUserLevel == "HQ") ? "" : $tSQL .= " AND CRDHD.FTBchCode = '$tCardShiftTopUpBchCode'";
                        $tSQL .= " AND CRDHD.FTCthDocType = '1'"; // 1: Top up    
    ...
    public function FSnMCardShiftTopUpGetPageAll($ptSearchList, $paData){
        ...
        $tSQL = "SELECT COUNT (CRDHD.FTBchCode) AS counts
                ...
                // BchCode is empty = HQ(use all)
                empty($tCardShiftTopUpBchCode) ? "" : $tSQL .= " AND CRDHD.FTBchCode = '$tCardShiftTopUpBchCode'";
                $tSQL .= " AND CRDHD.FTCthDocType = '1'"; // 1: Top up
    ...
    public function FSoMCardShiftTopUpCheckDuplicate($ptCardShiftTopUpCode){
        $tSQL = "SELECT COUNT(FTCthDocNo) AS counts
                    FROM TFNTCrdTopUpHD
                    WHERE FTCthDocNo = '$ptCardShiftTopUpCode'";
        $oQuery = $this->db->query($tSQL);
        if($oQuery->num_rows() > 0){
            return $oQuery->result();
        }else{
    ...
    public function FSaMCardShiftTopUpGetDTByDocNo($paData){
        ...
        $nLngID     = $paData['FNLngID'];
        $tSQL = "SELECT
                    CRDDT.FTCthDocNo AS rtCardShiftTopUpDocNo,
                    CRDDT.FTBchCode AS rtCardShiftTopUpBchCode,
                    CRDDT.FTCrdCode AS rtCardShiftTopUpCrdCode,
                    CRDDT.FNCtdSeqNo AS rtCardShiftTopUpSeqNo,
                    CRDDT.FTCtdStaCrd AS rtCardShiftTopUpStaCrd,
                    CRDDT.FCCtdCrdTP AS rtCardShiftTopUpCrdTP,
                    CRDDT.FTCtdRmk AS rtCardShiftTopUpRmk,
                    CRD.FTCrdStaShift AS rtCardShiftTopUpCrdStaShift,
                    CRD.FTCrdStaActive AS rtCardShiftTopUpCrdStaActive,
                    CRD.FTCrdStaType AS rtCardShiftTopUpCrdStaType,
                ...
                AND CRDDT.FTCthDocNo = '$tCardShiftTopUpDocNo'";
    ...
    public function FSaMCardShiftTopUpGetCardById($paData){
        ...
        $tSQL = "SELECT
                    ...
                    CRD.FTCrdStaType AS rtCardShiftTopUpStaType,
