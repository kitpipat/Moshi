...
class mDeposit extends CI_Model
{
    ...
    public function FSaMGetHD($paParams = [])
    {
        ...
        $tSQL = "
            SELECT
                ...
                USRL.FTUsrName AS FTCreateByName,
                USRSEN.FTUsrCode AS tSenderCode,
                USRSEN.FTUsrName AS tSenderName,
                BBKL.FTBbkName,
                BBK.FTBbkType,
                BNKL.FTBnkName,
            ...
            LEFT JOIN TCNMUser_L USRSEN WITH (NOLOCK) ON HD.FTBdhUsrSender = USRSEN.FTUsrCode AND USRSEN.FNLngID = $nLngID
            LEFT JOIN TFNMBookBank BBK WITH(NOLOCK) ON BBK.FTBbkCode = HD.FTBbkCode AND HD.FTBchCode = BBK.FTBchCode
            LEFT JOIN TFNMBookBank_L BBKL WITH(NOLOCK) ON BBK.FTBbkCode = BBKL.FTBbkCode AND BBK.FTBchCode = BBKL.FTBchCode AND BBKL.FNLngID = $nLngID
    ...
    public function FSaMDocCancel($paParams = [])
    {
        try {
            ...
            $this->db->set('FTRsnCode', $paParams['tRsnCode']);
            $this->db->set('FTLastUpdBy', $paParams['FTLastUpdBy']);
            $this->db->set('FDLastUpdOn', $paParams['FDLastUpdOn']);
