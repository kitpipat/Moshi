...
class mPdtGroup extends CI_Model {
    ...
    public function FSaMPGPList($paData){
        try{
            ...
            $tSQL           = "SELECT c.* FROM(
                                    SELECT  ROW_NUMBER() OVER(ORDER BY rtCreateOn DESC, rtPgpCode ASC) AS rtRowID,* FROM
                                        (SELECT DISTINCT
