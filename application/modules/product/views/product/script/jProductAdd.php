    ...
    var oPdtBrowseMerchantGroup = function(poReturnInput){
        ...
        var oOptionReturn       = {
            ...
            GrideView:{
                ...
                WidthModal          : 50,
                Perpage			    : 20,
                OrderBy			    : ['TCNMMerPdtGrp.FTMgpCode ASC'],
            },
            CallBack:{
                ReturnType	: 'S',
...
/** =================================================== Function And Event Pack Size ================================================ */
    ...
    var oPdtBrowseShop      = function(poReturnInput){
        ...
        var oOptionReturn       = {
            ...
            Join :{
                ...
                On: ['TCNMShop_L.FTShpCode = TCNMShop.FTShpCode AND TCNMShop_L.FNLngID = '+nLangEdits]//'TRTMPdtRental.FTShpCode = TCNMShop.FTShpCode'
            },
            Where:{
                Condition: [
                    tSQLWhere
    ...
    $('#obtBrowsePdtInfoShp').click(function(e){
        ...
        if($("#oetPdtBchCode").val() == '' && $("#oetPdtMerCode").val() != ''){
            tSQLWhere = "AND TCNMShop.FTMerCode = '"+$("#oetPdtMerCode").val()+"'";
    ...
    function JSxAddDataUnitPackSizeToTable(poDataNextFunc){
        ...
        $.ajax({
            ...
            success: function(tResult){
                // console.log(tResult);
                JsxCallPackSizeDataTable($('#oetPdtCode').val());
    ...
    function JSxPdtDelPszUnitInTable(oEvent){
        ...
        if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
            ...
            var nPdtCode    = $(oEvent).parents('.xWPdtDataUnitRow').data('pdtcode');

            $.ajax({
                type: "POST",
                url: "productDelPackSizeUnit",
                data:{ 
                    FTPunCode : $(oEvent).parents('.xWPdtDataUnitRow').data('puncode'),
                    FTPdtCode : $(oEvent).parents('.xWPdtDataUnitRow').data('pdtcode')
                },
                cache: false,
                timeout: 0,
                async: false,
...
/** =================================================== Function And Event BarCode ================================================== */
    ...
    // $('#obtModalAebBarCodeSubmit').off('click');
    // $('#obtModalAebBarCodeSubmit').on('click',function(){
    //     var nStaSession = JCNxFuncChkSessionExpired();
    //     if(typeof(nStaSession) !== 'undefined' && nStaSession == 1){
    //         // JSxPdtSaveBarCodeInUnitPack();
    //         alert('Test');
    ...
    function JSxPdtSaveBarCodeInUnitPack(){
        $('#ofmModalAebBarCode').validate({
            ...
            submitHandler: function(form){
                ...
                if($('#ocbModalAebBarStaUse').prop("checked")==true){ tBarStaUse = '1'; }else{ tBarStaUse = ''; }
                if($('#ocbModalAebBarStaAlwSale').prop("checked")==true){ tBarStaAlwSale = '1'; }else{ tBarStaAlwSale = ''; }
                if($('#ocbModalAesSplStaAlwPO').prop("checked")==true){ tSplStaAlwPO = '1'; }else{ tSplStaAlwPO = ''; }
            ...
            var nStaSession = JCNxFuncChkSessionExpired();
                if (typeof(nStaSession) !== 'undefined' && nStaSession == 1) {
                    ...
                    $('#ofmAddStockConditions').validate({
                        rules: {
                            oetStockConditionhBchName: { "required": {} },
                            oetStockConditionhWahName: { "required": {} },
                            oetStockConditionsMin: { "required": {} },
                            oetStockConditionsMax: { "required": {} },
                        },
                        messages: {
